...
local apartmentTargets = {}
local entranceThreadRunning = false
...
local function createEntrances()
    if entranceThreadRunning then return end
    entranceThreadRunning = true

    CreateThread(function()
        local currentNearId = nil

        while entranceThreadRunning do
            local sleep = 500
            local pedCoords = GetEntityCoords(cache.ped)

            local nearId = nil
            for id, data in pairs(sharedConfig.locations) do
                if #(pedCoords - data.enter.xyz) < 2.5 then
                    nearId = id
                    sleep = 0
                    break
                end
            ...
            if nearId then
                currentEntrance = nearId
                currentNearId = nearId
                lib.showTextUI(Lang:t('text.door_outside'), {
                    position = 'left-center'
                })

                if IsControlJustPressed(0, 38) then
                    showEntranceHeaderMenu(currentNearId)
                end
            else
                if currentNearId then
                    lib.hideTextUI()
                    currentNearId = nil
                    currentEntrance = nil
                end
...
local function removeEntrances()
    entranceThreadRunning = false
    lib.hideTextUI()
    currentEntrance = nil
end

local insideThreadRunning = false

local function createInsidePoints(id, data)
    apartmentZones[id] = apartmentZones[id] or {}

    insideThreadRunning = true
    CreateThread(function()
        local currentNearExit = false

        while insideThreadRunning do
            local sleep = 500
            local pedCoords = GetEntityCoords(cache.ped)

            local exitCoords = vector3(
                sharedConfig.locations[currentApartment].enter.x - data.exit.x,
                sharedConfig.locations[currentApartment].enter.y - data.exit.y - 0.5,
                sharedConfig.locations[currentApartment].enter.z - currentOffset + data.exit.z
            )

            if #(pedCoords - exitCoords) < 1.5 then
                sleep = 0
                currentNearExit = true
                lib.showTextUI(Lang:t('text.door_outside'), {
                    position = 'left-center'
                })
                ...
                if currentNearExit then
                    lib.hideTextUI()
                    currentNearExit = false
                end
            end
    ...
    local stashCoords = vector3(
        sharedConfig.locations[currentApartment].enter.x - data.stash.x,
        sharedConfig.locations[currentApartment].enter.y - data.stash.y,
        sharedConfig.locations[currentApartment].enter.z - currentOffset + data.stash.z
    )

    local clothesCoords = vector3(
        sharedConfig.locations[currentApartment].enter.x - data.clothes.x,
        sharedConfig.locations[currentApartment].enter.y - data.clothes.y,
        sharedConfig.locations[currentApartment].enter.z - currentOffset + data.clothes.z
    )

    apartmentTargets.stash = exports.ox_target:addSphereZone({
        coords = stashCoords,
        radius = 1.0,
        debug = false,
        distance = 2.0,
    ...
    apartmentTargets.clothes = exports.ox_target:addSphereZone({
        coords = clothesCoords,
        radius = 1.0,
        debug = false,
        options = {
            {
                ...
                onSelect = function()
                    ...
                    exports.ox_inventory:openInventory('stash', currentApartment .. '_clothes')
                end
            }
        }
    })
...
local function removeInsidePoints(id)
    insideThreadRunning = false
    lib.hideTextUI()

    if apartmentTargets.stash then
        exports.ox_target:removeZone(apartmentTargets.stash)
        apartmentTargets.stash = nil
    end
    if apartmentTargets.clothes then
        exports.ox_target:removeZone(apartmentTargets.clothes)
        apartmentTargets.clothes = nil
    end
end
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment, ownerCid)
    ...
    if new then
        CreateThread(function()
            Wait(1500)
            local enter = sharedConfig.locations[apartment].enter
            SetEntityCoords(cache.ped, enter.x - poiOffsets.exit.x, enter.y - poiOffsets.exit.y, enter.z - currentOffset + poiOffsets.exit.z, false, false, false, false)
            SetEntityHeading(cache.ped, poiOffsets.exit.h)
        end)
    end
end)
