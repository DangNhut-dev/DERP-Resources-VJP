...
RegisterNetEvent('spy-bodycam:bodycamstatus',function()
    ...
    local acvstring
    if bcamstate then acvstring = 'Đang tắt' else acvstring = 'Đang bật' end
...
RegisterNetEvent('spy-bodycam:toggleCarCam', function()
    ...
    if DoesEntityExist(veh) then 
        ...
        local acvstring = GlobalState.CarsOnBodycam[netId] and 'Đang tắt' or 'Đang bật'
        local donestr = GlobalState.CarsOnBodycam[netId] and 'Đã tắt' or 'Đã bật'
...
RegisterNetEvent('spy-bodycam:openActiveMenuJob', function(locId)
    local optionsMenu = {}
    if Config.Dependency.UseMenu == 'qb' then
        optionsMenu[#optionsMenu+1] = { header = 'Bodycam Sĩ Quan',isMenuHeader = true}
        optionsMenu[#optionsMenu+1] = { icon = "fas fa-circle-xmark", header = "Đóng", params = { event = "spy:close" } }
        optionsMenu[#optionsMenu+1] = { 
    ...
    elseif Config.Dependency.UseMenu == 'esx' then
        ...
        optionsMenu[#optionsMenu+1] = {icon="fas fa-left-long", title="Quay lại", name = 'back'}
    end
...
RegisterNetEvent('spy-bodycam:openActiveMenuCars', function(locId)
    local optionsMenu = {}
    if Config.Dependency.UseMenu == 'qb' then
        optionsMenu[#optionsMenu+1] = { header = 'Bodycam Xe Sĩ Quan',isMenuHeader = true}
        optionsMenu[#optionsMenu+1] = { icon = "fas fa-circle-xmark", header = "Close", params = { event = "spy:close" } }
        optionsMenu[#optionsMenu+1] = { 
            icon = "fas fa-left-long", 
            header = "Quay lại", 
    ...
    for k, v in pairs(GlobalState.CarsOnBodycam) do
        if Config.WatchLoc[locId].carCam then  
            ...
            if Config.Dependency.UseMenu == 'ox' then 
                optionsMenu[#optionsMenu+1] = {
                    title = v.carname,
                    description = "Biển số: "..v.plate.." | "..v.name,
                    icon = 'car',
                    onSelect = function()
                        StartDashCamWatch(k,v.plate,v.carname) 
                    end
