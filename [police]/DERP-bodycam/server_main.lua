...
RegisterCommand("records",function(playerId)
    if playerId then
        ...
        if not jobCheck then
            Notify(playerId, "Bạn không biết cách sử dụng.")
            return
        end
        TriggerClientEvent("DERP-bodycam:action",playerId, "records", Videos)
    end
end)
    ...
    if playerId then
        ...
        if player then
            ...
            if not itemCheck then
                Notify(playerId, "Bạn không có máy quay.")
                return
            end
            TriggerClientEvent("DERP-bodycam:action", playerId, "bodycam", info)
