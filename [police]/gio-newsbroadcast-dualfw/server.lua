...
RegisterCommand('announce', function(source, args)
    ...
    if not cfg then
        Notify(source, 'Loại không hợp lệ! Hãy sử dụng: ambulance, police, gov hoặc event')
        return
    end
    ...
    if #message < 5 then
        Notify(source, 'Quá ngắn!')
        return
    end

    if #message > 300 then
        Notify(source, 'Quá dài, tối đã chỉ 300 từ.')
        return
    end
    ...
    if cfg.job then
        ...
        if not job then
            Notify(source, 'Không thể truy xuất dữ liệu của bạn. Vui lòng thử lại.')
            return
        end

        if job ~= cfg.job then
            Notify(source, 'Bạn không được phép xem loại thông báo này!')
            return
        end

        -- Grade check (skip if grade = 0)
        if cfg.grade and cfg.grade > 0 and grade < cfg.grade then
            Notify(source, 'Bạn không đạt đủ cấp bậc cần thiết cho thông báo này!')
            return
        end
    else
        -- No job required — enforce ACE permission if configured
        if cfg.ace and not IsPlayerAceAllowed(source, cfg.ace) then
            Notify(source, 'Bạn không được phép hiển thị loại thông báo này!')
            return
        end
    end
    ...
    local license = GetLicense(source)
    if not license then
        Notify(source, 'Không thể xác minh danh tính của bạn. Vui lòng thử lại.')
        return
    end
    ...
    if cfg.cooldown and cfg.cooldown > 0 then
        ...
        if remaining > 0 then
            Notify(source, ('Bạn phải đợi thêm %d giây nữa trước khi phát sóng lại.'):format(remaining))
            return
        end
...
RegisterCommand('clearticker', function(source, _)
    if source ~= 0 and not IsPlayerAceAllowed(source, 'command.clearticker') then
        Notify(source, 'Bạn không được phép sử dụng lệnh này!')
        return
    end
