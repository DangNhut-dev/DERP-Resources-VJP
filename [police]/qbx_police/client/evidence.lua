...
local function dropBulletCasing(weapon, ped)
    ...
    -- print('[DEBUG] currentWeapon:', json.encode(currentWeapon))
    local serial = currentWeapon?.metadata?.serial
    local ammoName = currentWeapon?.ammo
    -- print('[DEBUG] ammoName:', ammoName)
    local ammoLabel = nil
    if ammoName then
        local ammoData = exports.ox_inventory:Items(ammoName)
        -- print('[DEBUG] ammoData:', json.encode(ammoData))
        if ammoData then ammoLabel = ammoData.label end
    end
    -- print('[DEBUG] ammoLabel:', ammoLabel)
    TriggerServerEvent('evidence:server:CreateCasing', weapon, serial, coords, ammoLabel)
    Wait(300)
end
...
RegisterNetEvent('evidence:client:AddCasing', function(casingId, weapon, coords, serie, ammoLabel)
    casings[casingId] = {
        type = weapon,
...
--     for name, data in pairs(exports.qbx_core:GetWeapons()) do
--         if GetHashKey(name) == weaponHash or (data.hash and data.hash == weaponHash) then
--             weaponName = name
--             break
--         end
--     end
...
--     local weaponData = exports.ox_inventory:Items(string.upper(weaponName))
--     print('[DEBUG] weaponData:', json.encode(weaponData))
--     print('[DEBUG] ammoname:', weaponData?.ammoname)

--     if not weaponData?.ammoname then return end

--     local ammo = exports.ox_inventory:Items(weaponData.ammoname)
--     print('[DEBUG] ammo:', json.encode(ammo))
        ...
        if currentCasing and currentCasing ~= 0 then
            drawEvidenceIfInRange({
                ...
                metadata = {
                    ...
                    ammolabel = casings[currentCasing].ammoLabel,
