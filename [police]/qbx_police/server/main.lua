...
lib.callback.register('police:GetPlayerStatus', function(_, targetSrc)
    ...
    if not player or not playerStatus[targetSrc] or not next(playerStatus[targetSrc]) then return {} end
...
RegisterNetEvent('evidence:server:CreateCasing', function(weapon, serial, coords, ammoLabel)
    ...
    TriggerClientEvent('evidence:client:AddCasing', -1, casingId, weapon, coords, serieNumber, ammoLabel)
end)
...
RegisterNetEvent('evidence:server:AddCasingToInventory', function(casingId, casingInfo)
    ...
    local player = exports.qbx_core:GetPlayer(src)
    local playerName = player.PlayerData.charinfo.firstname .. ' ' .. player.PlayerData.charinfo.lastname
    local streetName = casingInfo.street or 'Unknown'
    local ammoType = casingInfo.ammolabel or casingInfo.ammotype or 'Unknown'
    local serialNumber = casingInfo.serie or 'N/A'
    local metadata = {}
    metadata.type = 'Casing Evidence'
    metadata.description = 'Loại Đạn: ' .. ammoType
    metadata.description = metadata.description .. '\n\nSố Serial: ' .. serialNumber
    metadata.description = metadata.description .. '\n\nThu Thập Bởi: ' .. playerName
    metadata.description = metadata.description .. '\n\nThu Thập Ở: ' .. streetName
    if not exports.ox_inventory:RemoveItem(src, 'empty_evidence_bag', 1) then
        return exports.qbx_core:Notify(src, locale('error.have_evidence_bag'), 'error')
    end
    if exports.ox_inventory:AddItem(src, 'filled_evidence_bag', 1, metadata) then
...
RegisterNetEvent('police:server:RobOpenInventory', function(targetSrc)
    local src = source
    ...
    local targetCoords = GetEntityCoords(targetPed)
    if #(srcCoords - targetCoords) > 3.0 then return end
 
    -- Check: canSteal OR death state bags (p_ambulancejob)
    local targetState = Player(targetSrc).state
    local canSteal = targetState.canSteal
    local isDead = targetState.isDead or targetState.dead
    local deathType = targetState.deathType
    local isInDeathState = isDead or (deathType and deathType ~= 'none')
 
    -- Check: còng qua metadata
    local targetPlayer = exports.qbx_core:GetPlayer(targetSrc)
    local isCuffed = targetPlayer and targetPlayer.PlayerData.metadata.ishandcuffed
 
    if not (canSteal or isInDeathState or isCuffed) then return end
