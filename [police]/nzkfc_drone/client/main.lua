...
local function spawnDroneProp(pos)
    ...
    if not HasModelLoaded(model) then
        lib.notify({ type = 'error', title = 'Drone', description = 'Không thể tải mô hình drone.' })
        return nil
    end
...
startBatteryThread = function()
    ...
    batteryThread = CreateThread(function()
        ...
        if not batterySlot then
            lib.notify({ type = 'error', title = 'Drone', description = 'Không tìm thấy pin trong kho drone! Hãy đặt drone_battery vào kho.' })
            ...
            if drain > 0 then
                ...
                if batteryCharge <= Config.BatteryLowThreshold then
                    ...
                    if batteryLowTimer >= Config.BatteryLowSoundEvery then
                        batteryLowTimer = 0
                        lib.notify({ type = 'warning', title = 'Drone', description = ('Pin yếu! Còn %d%% — hãy thay pin trong kho drone.'):format(math.floor(batteryCharge)) })
                    end
                end
...
local function landAndGround()
    ...
    lib.notify({ type = 'error', title = 'Drone', description = 'Hết pin! Mở kho drone và lắp drone_battery vào.' })
...
wakeUpDrone = function()
    ...
    lib.notify({ type = 'success', title = 'Drone', description = 'Đã lắp pin — drone đang khởi động lại!' })
        ...
        while droneActive and droneEntity and DoesEntityExist(droneEntity) and Config.DamageEnabled do
            ...
            if HasEntityBeenDamagedByWeapon(droneEntity, 0, 2) then
                ...
                if droneHealth > 0 then
                    lib.notify({ type = 'warning', title = 'Drone', description = ('Drone bị bắn! Máu: %d/%d'):format(droneHealth, Config.DroneMaxHealth) })
                    TriggerServerEvent('nzkfc_drone:saveHealth', droneSerial, droneHealth)
                else
                    lib.notify({ type = 'error', title = 'Drone', description = 'Drone đã bị phá hủy! Kho vẫn có thể thu hồi.' })
...
local function deployDrone(itemMeta, slot)
    ...
    lib.notify({ type = 'inform', title = 'Drone', description = 'Đang triển khai drone...' })
    ...
    DroneTargeting.Add(
        ...
        function()
            if guardActive then
                ...
                lib.notify({ type = 'inform', title = 'Drone', description = 'Đã tắt chế độ bảo vệ.' })
            else
                guardActive = true
                ...
                lib.notify({ type = 'error', title = 'Drone', description = 'Đã bật chế độ bảo vệ! Drone sẽ bắn kẻ thù xung quanh.' })
            end
        end,
        ...
        function()
            if healActive then
                -- Turn off
                healActive = false
                lib.notify({ type = 'inform', title = 'Drone', description = 'Đã tắt chế độ hồi máu.' })
            else
                -- Turn on — immediately fire first tick then let timer handle the rest
                healActive = true
                TriggerServerEvent('nzkfc_drone:healNearby', GetEntityCoords(droneEntity))
                lib.notify({ type = 'success', title = 'Drone', description = 'Đã bật chế độ hồi máu.' })
            end
        end,
        ...
        function()
            if droneStaying then
                ...
                droneStaying = false
                stayPos      = nil
                lib.notify({ type = 'inform', title = 'Drone', description = 'Drone đang theo bạn trở lại.' })
            else
                droneStaying = true
                stayPos      = GetEntityCoords(droneEntity)
                lib.notify({ type = 'inform', title = 'Drone', description = 'Drone đứng yên tại chỗ. Dùng /calldrone để gọi lại.' })
            end
        end,
        ...
        function()
            if DroneControl.IsControlling() then
                lib.notify({ type = 'warning', title = 'Drone', description = 'Đang ở chế độ điều khiển rồi.' })
                return
            end
            droneStaying = false
        ...
        function()
            if not batteryEnabled then
                lib.notify({ type = 'inform', title = 'Drone', description = 'Hệ thống pin đã bị tắt.' })
                return
            end
            if not batteryReady then
                lib.notify({ type = 'inform', title = 'Drone', description = 'Đang đọc thông tin pin...' })
                return
            end
        ...
        function()
            ...
            if flightSoundMuted then
                stopFlySound()
                lib.notify({ type = 'inform', title = 'Drone', description = 'Đã tắt âm thanh động cơ.' })
            else
                if droneEntity and DoesEntityExist(droneEntity) then
                    startFlySound(droneEntity)
                end
                lib.notify({ type = 'inform', title = 'Drone', description = 'Đã bật âm thanh động cơ.' })
            end
        end,
        ...
        function()
            droneLight = not droneLight
            local state = droneLight and 'on' or 'off'
            lib.notify({ type = 'inform', title = 'Drone', description = ('Đèn chiếu %s.'):format(state) })
        end
    )
    ...
    lib.notify({ type = 'success', title = 'Drone', description = ('Drone đã triển khai! [%s] Máu: %d/%d'):format(droneSerial, droneHealth, Config.DroneMaxHealth) })
end
...
local function undeployDrone()
    ...
    lib.notify({ type = 'inform', title = 'Drone', description = 'Đang gọi drone về...' })
    ...
    lib.notify({ type = 'success', title = 'Drone', description = 'Đã cất drone vào túi.' })
end
...
RegisterCommand('calldrone', function()
    if not droneActive then
        lib.notify({ type = 'error', title = 'Drone', description = 'Không có drone nào đang hoạt động.' })
        return
    end
    if droneStaying then
        droneStaying = false
        stayPos      = nil
        lib.notify({ type = 'success', title = 'Drone', description = 'Drone đang quay lại với bạn.' })
    else
        lib.notify({ type = 'inform', title = 'Drone', description = 'Drone đang theo bạn rồi.' })
    end
end, false)
...
local function handleItemUse(data)
    ...
    if not droneActive and Config.JobRestrict and not Framework.hasJob(Config.JobRestrict) then
        lib.notify({ type = 'error', title = 'Drone', description = 'Bạn không có quyền sử dụng thiết bị này.' })
        return
    end
...
RegisterNetEvent('nzkfc_drone:serialAssigned', function(metadata)
    itemUseLock = false
    lib.notify({ type = 'success', title = 'Drone', description = 'Drone mã số: ' .. metadata.serial })
...
RegisterNetEvent('nzkfc_drone:healComplete', function()
    if healActive then
        healActive = false
        lib.notify({ type = 'success', title = 'Drone', description = 'Tất cả người chơi đã được hồi máu.' })
    end
end)
