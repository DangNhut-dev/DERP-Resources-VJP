...
RegisterKeyMapping('+drone_descend', 'Drone: Hạ độ cao (giữ E)', 'keyboard', 'e')
...
RegisterKeyMapping('+drone_light', 'Drone: Bật/Tắt đèn chiếu', 'keyboard', 'l')
...
function DroneControl.Start(droneEntity, startPos)
    ...
    -- Show control hint
    lib.notify({ type = 'inform', title = 'Điều Khiển Drone', description = 'W/S/A/D = Di chuyển | Q = Lên | E = Xuống | SPACE = Ngắt kết nối' })
    ...
    CreateThread(function()
        while controlling do
            ...
            if dist > Config.ControlMaxRange then
                lib.notify({ type = 'error', title = 'Drone', description = 'Mất tín hiệu — drone đang quay lại.' })
...
function DroneControl.Stop(droneEntity)
    ...
    lib.notify({ type = 'success', title = 'Drone', description = 'Đã ngắt kết nối khỏi drone.' })
end
