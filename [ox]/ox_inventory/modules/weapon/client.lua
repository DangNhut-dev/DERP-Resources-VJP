...
function Weapon.Equip(item, data, noWeaponAnim)
    ...
    -- Skin replacement: dùng skin hash thay original hash
    local skinHash = WeaponSkinClient.getEquipHash(data.hash)
    local equipHash = skinHash or data.hash
    ...
    item.hash = equipHash  -- dùng skin hash để tick không force disarm
    item.ammo = data.ammoname
    item.melee = GetWeaponDamageType(equipHash) == 2 and 0
    item.timer = 0
    item.throwable = data.throwable
    item.group = GetWeapontypeGroup(equipHash)

    GiveWeaponToPed(playerPed, equipHash, 0, false, true)

    if item.metadata.tint then SetPedWeaponTintIndex(playerPed, equipHash, item.metadata.tint) end
    ...
    if item.metadata.components then
        for i = 1, #item.metadata.components do
            ...
            for v = 1, #components do
                ...
                if DoesWeaponTakeWeaponComponent(equipHash, component) then
                    if not HasPedGotWeaponComponent(playerPed, equipHash, component) then
                        GiveWeaponComponentToPed(playerPed, equipHash, component)
                    end
                end
            end
        end
    ...
    if item.metadata.specialAmmo then
        ...
        if DoesWeaponTakeWeaponComponent(equipHash, specialClip) then
            GiveWeaponComponentToPed(playerPed, equipHash, specialClip)
        end
    end

    local ammo = item.metadata.ammo or item.throwable and 1 or 0

    SetCurrentPedWeapon(playerPed, equipHash, true)
    SetPedCurrentWeaponVisible(playerPed, true, false, false, false)
    SetWeaponsNoAutoswap(true)
    SetPedAmmo(playerPed, equipHash, ammo)
    ...
    if item.group == `GROUP_PETROLCAN` or item.group == `GROUP_FIREEXTINGUISHER` then
        ...
        SetPedInfiniteAmmo(playerPed, true, equipHash)
    end
