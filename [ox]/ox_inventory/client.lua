...
local WeaponSkinClient = require 'modules.weaponskin.client'

RegisterNUICallback('cloth-slot', function(_, cb)
    SendNUIMessage({ action = 'showClothSlot' })
    cb('ok')
end)

RegisterNUICallback('drop', function(_, cb)
    SendNUIMessage({ action = 'showDropInventory' })
    cb('ok')
end)

RegisterNUICallback('weapon-skin', function(_, cb)
    SendNUIMessage({ action = 'showWeaponSkin' })
    cb('ok')
end)
 
RegisterNUICallback('requestWeaponSkinSync', function(_, cb)
    TriggerServerEvent('ox_inventory:requestWeaponSkinSync')
    cb('ok')
end)
 
RegisterNUICallback('equipWeaponSkin', function(data, cb)
    ...
    local success = lib.callback.await('ox_inventory:equipWeaponSkin', false, data.fromSlot, data.toSlot)
    cb(success or false)
end)

RegisterNUICallback('unequipWeaponSkin', function(data, cb)
    if not data or not data.skinSlot then return cb(false) end
    if currentWeapon then
        lib.notify({ type = 'error', description = locale('cannot_perform') })
        return cb(false)
    end
    local success = lib.callback.await('ox_inventory:unequipWeaponSkin', false, data.skinSlot, data.toSlot)
    cb(success or false)
end)

RegisterNuiCallback('equipWeaponSkinFromBackpack', function(data, cb)
    local fromSlot = tonumber(data.fromSlot)
    local toSlot   = tonumber(data.toSlot)
    if not fromSlot or not toSlot then return cb(false) end

    local success = lib.callback.await('ox_inventory:equipWeaponSkinFromBackpack', false, fromSlot, toSlot)
    cb(success)
end)

RegisterNUICallback('moveSkinSlot', function(data, cb)
    if not data or not data.fromSlot or not data.toSlot then return cb(false) end
    local success = lib.callback.await('ox_inventory:moveSkinSlot', false, data.fromSlot, data.toSlot)
    cb(success or false)
end)

RegisterNUICallback('unequipWeaponSkinToBackpack', function(data, cb)
    local skinSlot = tonumber(data.skinSlot)
    local toSlot   = tonumber(data.toSlot)
    if not skinSlot or not toSlot then return cb(false) end
    if currentWeapon then
        lib.notify({ type = 'error', description = locale('cannot_perform') })
        return cb(false)
    end
    local success = lib.callback.await('ox_inventory:unequipWeaponSkinToBackpack', false, skinSlot, toSlot)
    cb(success)
end)
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	SendNUIMessage({
		...
		data = {
			...
			clothesImagepath = client.clothesImagepath,
			rarityConfig = RarityConfig.GetNUIData(), 
		}
	})
...
RegisterNUICallback('requestClothSync', function(_, cb)
...
local function cleanupGiveState()
    local ped = cache.ped
    if giveState.prop and DoesEntityExist(giveState.prop) then
        DeleteEntity(giveState.prop)
    end
    giveState.active = false
    giveState.prop = nil
    giveState.slot = nil
    giveState.count = nil
    ClearPedTasks(ped)
...
--- DrawText3D phía trên đầu target
...
RegisterNUICallback('giveItem', function(data, cb)
    ...
    if usingItem or giveState.active then return end
    ...
    -- Spawn prop trên tay phải
    local propModel = `prop_paper_bag_small`
    lib.requestModel(propModel)

    local propEntity = CreateObject(propModel, 0.0, 0.0, 0.0, true, true, false)
    local boneIndex = GetPedBoneIndex(ped, 57005) -- SKEL_R_Hand

    AttachEntityToEntity(propEntity, ped, boneIndex,
        0.10, 0.04, -0.04,
        -90.0, 0.0, 0.0,
    ...
    giveState.active = true
    giveState.prop = propEntity
    giveState.slot = slot
    giveState.count = count

    lib.showTextUI('[X] Để hủy', { position = 'left-center', icon = 'xmark' })

    CreateThread(function()
        while giveState.active do
            Wait(0)

            -- ── Cancel: X (73) | mở inventory | đang dùng item ──
            if IsControlJustPressed(0, 73) or invOpen or usingItem then
                cleanupGiveState()
                return
            end
            ...
            for i = 1, #players do
                ...
                if playerId ~= cache.playerId then
                    ...
                    if targetPed ~= 0 and IsEntityVisible(targetPed) then
            ...
            -- ── Hiển thị "[E] Đưa" + xác nhận ──
            if closestPed and closestServerId then
                local targetCoords = GetEntityCoords(closestPed)
                DrawText3D(targetCoords + vec3(0.0, 0.0, 1.0), '[~g~E~w~] Đưa')

                if IsControlJustPressed(0, 38) then -- E
                    cleanupGiveState()
                    giveItemToTarget(closestServerId, slot, count)
                    return
                end
            end
        end
...
RegisterNUICallback('backpackDropItem', function(data, cb)
    if swapActive or not invOpen or invBusy then return cb(false) end
    ...
    if IsEntityInWater(playerPed) then
        ...
        while true do
            ...
            if retval ~= 1 then
                if not hit then return cb(false) end
    ...
    TriggerServerEvent('ox_inventory:backpackDrop', {
        fromSlot = data.fromSlot,
        toSlot = data.toSlot or 1,
        count = data.count,
        coords = coords,
