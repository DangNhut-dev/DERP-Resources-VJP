...
import { clothesImagepath, imagepath } from '../store/imagepath';
...
export const getItemUrl = (item: string | SlotWithItem) => {
  ...
  if (isObj) {
    ...
    if (metadata?.drawableId !== undefined && metadata?.textureId !== undefined && metadata?.gender !== undefined) {
      return `${clothesImagepath}/${item.name}_${metadata.drawableId}_${metadata.textureId}_${metadata.gender}.png`;
    }
  }
