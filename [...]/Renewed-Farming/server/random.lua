...
CreateThread(function()
    ...
    for i = 1, #Config.Locations do
        ...
        if not Plants[Location.type] then
            ...
            if Config.minusOne then
                Location.coords = vec3(Location.coords.x, Location.coords.y, Location.coords.z + -1.5 + pData.offset.z)
            else
    ...
    for i = 1, Config.maxSpots do
        local amount = #Config.Locations

        if amount == 0 then break end

        local random = math.random(1, #Config.Locations)
        local Location = random and Config.Locations[random]
        ...
        if Location then
            ...
            table.remove(Config.Locations, random)
        end
    end
end)
...
RegisterNetEvent('Renewed-Farming:server:harvestRandoms', function(id)
    ...
    if Location then
        ...
        TriggerClientEvent('Renewed-Farming:client:removeRandom', -1, id)
    end
end)
