...
JS17Movement.GetTranslation = (key, args) => {
	let translation = JS17Movement.Lang[key] ?? "Không xác định";
...
window.addEventListener("message", function (event) {
	...
	if (action === "addNewMember") {
		...
		$("#startBtn").text("Kết Thúc Công Việc")
		$("#decoLineStart").text("QUẢN LÝ")
	} else if (action === "startBtnBackToNormal") {
		$("#startBtn").text("Bắt Đầu Công Việc")
		$("#decoLineStart").text("Bắt Đầu")
