...
AddEventHandler("drc_houserobbery:reset", function(house)
    if AllObjects[house] then
        for _, v in pairs(AllObjects[house]) do
        ...
        AllObjects[house] = nil
    end
    if CreatedProps[house] then
        for _, v in pairs(CreatedProps[house]) do
            SetEntityAsMissionEntity(v, false, true)
            DeleteObject(v)
            if DoesEntityExist(v) then
                TriggerServerEvent("drc_houserobbery:removeobject", ObjToNet(v))
...
function enterHouse(house, locked)
    ...
    if locked then
        ...
        if Config.InteractionType == 'target' then
            TriggerServerEvent("drc_houserobbery:lockpick", house)
            return
        end
...
function leaveHouse(house)
    for _, v in pairs(CreatedProps[house] or {}) do
        SetEntityAsMissionEntity(v, false, true)
        DeleteObject(v)
    end
    local attachentity
...
AddEventHandler("drc_houserobbery:SpawnPed", function(house, exist)
    ...
    if not PedSpawned[house] then
        ...
        local pedConfig = Config.HousesToRob[house].Residence.Ped
        local pedList = pedConfig[1] and pedConfig or { pedConfig }

        for _, pedData in ipairs(pedList) do
            if math.random(100) <= pedData.chance then
                local Attack = false
                RequestModel(pedData.model)
                while not HasModelLoaded(pedData.model) do
                    Wait(100)
                end
                NPC = CreatePed(4, pedData.model,
                    pedData.coords.x, pedData.coords.y,
                    pedData.coords.z, pedData.coords.w, true, true)
                if pedData.weapon.enabled then
                    if math.random(100) <= pedData.weapon.chance then
                        GiveWeaponToPed(NPC, pedData.weapon.weapon, 90, true, true)
                        if pedData.weapon.DisableWeaponDrop then
                            SetPedDropsWeaponsWhenDead(NPC, false)
                        end
                    end
                end
                TaskStartScenarioInPlace(NPC, "WORLD_HUMAN_BUM_SLUMPED", 0, true)
                if not AllObjects[house] then AllObjects[house] = {} end
                if not CreatedProps[house] then CreatedProps[house] = {} end
                table.insert(AllObjects[house], NPC)
                table.insert(CreatedProps[house], NPC)
                local npcRef = NPC
                SetTimeout(1100, function()
                    if DoesEntityExist(npcRef) then
                        TriggerServerEvent('drc_houserobbery_instance:addEntityToPlayerInstance', PedToNet(npcRef))
                    end
                end)
                local pedAlive = true
                CreateThread(function()
                    local localNPC = npcRef
                    while pedAlive and DoesEntityExist(localNPC) do
                        if Attack then
                            TaskCombatPed(localNPC, cache.ped, 0, 16)
                        end
                        if currentHouse and (IsPedJumping(cache.ped) and not Attack) or (IsPedShooting(cache.ped) and not Attack) or (IsPedRunning(cache.ped) and not Attack) or (GetEntityPlayerIsFreeAimingAt(PlayerId(-1)) == localNPC and not Attack) then
                            Attack = true
...
AddEventHandler("drc_houserobbery:lockpick", function(house)
    ...
    if success then
        ...
        if math.random(100) <= Residence.ReportChanceWhenEntering then
            print('[DRC DEBUG] alarm triggered | chance config:', Residence.ReportChanceWhenEntering)
            Dispatch(Config.HousesToRob[house].Coords.xyz, "houserobbery")
            AlarmSound()
...
function createZones(house)
    ...
    if Config.InteractionType ~= 'target' then
        ...
        SetTimeout(2000, function()
            for k, v in pairs(Config.HousesToRob[house].Residence.CreateProps) do
                ...
                if not AlreadyRobbed then
                    ...
                    if not AllObjects[house] then AllObjects[house] = {} end
                    if not CreatedProps[house] then CreatedProps[house] = {} end
                    table.insert(AllObjects[house], object)
                    table.insert(CreatedProps[house], object)
                    ...
                    lib.zones.sphere({
                        ...
                        inside = function(self)
                            if IsControlJustReleased(0, 38) and not doingAction then
                                ...
                                SetTimeout(500, function()
                                    house = currentHouse
                                    if not HouseHacked[currentHouse] and not HackFailed[currentHouse] and HaveLaptop then
                                        Hacking = true
                                        doAnimation()
                                        FreezeEntityPosition(cache.ped, true)
                                        if HackingMinigame() then
                                            ...
                                            TriggerServerEvent("drc_houserobbery:AddHackedHouse", currentHouse)
                                        else
                                            HackFailed[currentHouse] = true
                                            AlarmSound()
                                            Dispatch(Config.HousesToRob[house].Coords.xyz, "houserobbery")
                                            -- Notify('error', locale('Alarm'), locale('startedalarm'))
                                            Notify('error', locale('error'), locale('failed'))
        ...
        for k, v in pairs(Config.HousesToRob[house].Residence.Lasers) do
            ...
            if (Config.HousesToRob[house].Residence.LaserChance or 0) > 0 and math.random(100) <= Config.HousesToRob[house].Residence.LaserChance and not HouseLasered[house] then
    ...
    elseif Config.InteractionType == 'target' then
                        ...
                        options = {
                            {
                                action = function()
                                    ...
                                    SetTimeout(500, function()
                                        if not doingAction and HaveLaptop then
                                            if not HouseHacked[currentHouse] and not HackFailed[currentHouse] then
                                                Hacking = true
                                                doAnimation()
                                                FreezeEntityPosition(cache.ped, true)
                                                if HackingMinigame() then
                                                    ...
                                                    HackFailed[currentHouse] = true
                                                    AlarmSound()
                                                    Dispatch(Config.HousesToRob[currentHouse].Coords.xyz, "houserobbery")
                                                    -- Notify('error', locale('Alarm'), locale('startedalarm'))
                                                    Hacking = false
                                                    FreezeEntityPosition(cache.ped, false)
                                                    Notify('error', locale('error'), locale('failed'))
                                                end
                                            elseif HouseHacked[currentHouse] then
                                                Notify('success', locale('success'), locale('AlreadyHacked'))
                                            end
                                        end
                                    end)
        ...
        for k, v in pairs(Config.HousesToRob[house].Residence.Lasers) do
            ...
            TriggerServerEvent("drc_houserobbery:CheckHackedHouse", house)
            TriggerServerEvent('drc_houserobbery:CheckPowderHouse', house)
            local laserChance = Config.HousesToRob[house].Residence.LaserChance or 0
            if laserChance > 0 and math.random(100) <= laserChance then
                if math.random(100) <= v.chance then
...
AddEventHandler('onResourceStop', function(resource)
    ...
    for h, objects in pairs(AllObjects) do
    ...
    for h, props in pairs(CreatedProps) do
