...
if (z.config.EnableStaffChatForwarding) {
    RegisterCommand("staff", (source, args, raw) => {
        ...
        staffChannel.send({ content: `${GetPlayerName(source)}: ${raw.substring(6)}`, allowMentions: false }).catch((e) => {
            z.utils.log.error("Có vẻ như tôi không có đủ quyền cần thiết để chuyển tiếp cuộc trò chuyện giữa Staff đến kênh Staff đã cấu hình");
        });
    }, false);
    ...
    RegisterCommand("stafftoggle", (source, args, raw) => {
        if (IsPlayerAceAllowed(source, "zdiscord.staffchat")) {
            ...
            z.utils.chatMessage(source, z.locale.staffchat, "Staff chat hủy.", { color: [ 255, 255, 0 ] });
        } else {
            const member = z.bot.getMemberFromSource(source);
            if (z.bot.isRolePresent(member, z.config.StaffChatRoleIds)) {
                ExecuteCommand(`add_principal "player.${source}" group.zdiscordstaff`);
                z.utils.chatMessage(source, z.locale.staffchat, "Staff chat bắt đầu.", { color: [ 255, 255, 0 ] });
            }
        }
    }, false);

    setImmediate(() => {
        emit("chat:addSuggestion", "/staff", "Gửi tin nhắn cho staff khác (Staff only)", [
            { name:"Message", help:"Gửi tin nhắn cho staff khác" },
        ]);
        emit("chat:addSuggestion", "/stafftoggle", "Chuyển đổi tin nhắn trò chuyện của staff", []);
    });
}
