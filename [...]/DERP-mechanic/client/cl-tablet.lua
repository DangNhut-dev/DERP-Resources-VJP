...
function hideTabletToShowInteractionPrompt(instructionText)
	...
	TriggerEvent("DERP-mechanic:client:tablet-hidden-for-interaction")
	SendNUIMessage({
...
function showTabletAfterInteractionPrompt()
	...
	TriggerEvent("DERP-mechanic:client:tablet-shown-after-interaction")
	SendNUIMessage({
...
function ConnectVehicle(vehicleData)
	...
	local success = lib.callback.await("DERP-mechanic:server:connect-vehicle", false, plate, netId)
	if not success then
...
function DisconnectVehicle()
	...
	lib.callback.await("DERP-mechanic:server:disconnect-vehicle", false, connectedVehicle.plate)
...
local function getNearbyVehicles()
	...
	for _, data in ipairs(foundVehicles) do
		...
		local mileage, mileageUnit = lib.callback.await("DERP-mechanic:server:get-vehicle-mileage", false, plate)
...
local function openTablet()
	...
	local availableMechanics = lib.callback.await("DERP-mechanic:server:get-player-mechanics", false)
	...
	local preferences = lib.callback.await("DERP-mechanic:server:get-tablet-preferences", false)
...
RegisterNUICallback("tablet-login", function(data, cb)
	...
	local mechanicData = lib.callback.await("DERP-mechanic:server:get-tablet-mechanic-data", false, mechanicId)
...
RegisterNUICallback("toggle-on-duty", function(data, cb)
	...
	local success = lib.callback.await("DERP-mechanic:server:toggle-on-duty", false, toggle)
	if not success then
		cb(false)
	end
...
RegisterNUICallback("save-preferences", function(data, cb)
	...
	local success = lib.callback.await("DERP-mechanic:server:save-tablet-settings", false, preferences)
	if not success then
		return cb(false)
	end
	cb(true)
...
RegisterNetEvent("DERP-mechanic:client:use-tablet", function()
	openTablet()
end)
