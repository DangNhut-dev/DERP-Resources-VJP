...
function openModsMenu(mechId, mechLabel)
  ...
  if Config.ChangePlateDuringPreview then
    lib.callback.await("DERP-mechanic:server:open-mods-menu", false, VehToNet(currentVehicle))
  end

  local isMechanicEmployee = lib.callback.await("DERP-mechanic:server:is-mechanic-employee", false, mechId)
  local mechanicBalance = isMechanicEmployee and lib.callback.await("DERP-mechanic:server:get-mechanic-balance", false, mechId) or false
  local mechanicsOnDuty = lib.callback.await("DERP-mechanic:server:count-currently-on-duty", false, mechId)
...
RegisterNetEvent("DERP-mechanic:client:open-customisation-menu", openModsMenu)
...
RegisterNUICallback("purchase-mods", function(data, cb)
  ...
  local success = lib.callback.await("DERP-mechanic:server:purchase-mods", false, currentMechanicId, vehicleValue, cart, paymentMethod)

  if success == false then
    return cb({ error = true })
  end

  local isMechanicEmployee = lib.callback.await("DERP-mechanic:server:is-mechanic-employee", false, currentMechanicId)
  if mechanicConfig.type == "self-service" or (Config.MechanicEmployeesCanSelfServiceMods and isMechanicEmployee) then
    ...
    lib.callback.await("DERP-mechanic:server:save-veh-statebag-data-to-db", false, vehiclePlate)
    isPurchaseFinalized = true
    lib.callback.await("DERP-mechanic:server:self-service-mods-applied", false, currentMechanicId, VehToNet(currentVehicle), vehiclePlate, cart, success, paymentMethod)
    if Config.UpdatePropsOnChange then
      lib.callback.await("DERP-mechanic:server:save-vehicle-props", false, vehiclePlate, currentProps)
    end
    ...
    local orderPlaced = lib.callback.await("DERP-mechanic:server:place-order", false, currentMechanicId, vehiclePlate, cart, success, appliedMods, paymentMethod)
...
RegisterNUICallback("repair-vehicle", function(data, cb)
  ...
  local vehicleValue = Framework.Client.GetVehicleValue(GetEntityArchetypeName(currentVehicle))
  local success = lib.callback.await("DERP-mechanic:server:self-service-repair-vehicle", false, currentMechanicId, vehicleValue, data and data.paymentMethod)
  if success then
