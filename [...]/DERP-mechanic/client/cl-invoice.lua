...
RegisterNUICallback("get-unpaid-invoices", function(data, cb)
  local invoices = lib.callback.await("DERP-mechanic:server:get-unpaid-invoices", false)
  cb(invoices)
end)
...
RegisterNUICallback("save-invoice", function(data, cb)
  ...
  local success = lib.callback.await("DERP-mechanic:server:save-invoice", false, invoiceItems, invoiceTotal)
  cb(success)
end)
...
RegisterNUICallback("send-invoice", function(data, cb)
  ...
  local success = lib.callback.await("DERP-mechanic:server:send-invoice", false, player, invoiceItems, invoiceTotal)
  cb(success)
end)
...
RegisterNUICallback("resend-invoice", function(data, cb)
  ...
  local success = lib.callback.await("DERP-mechanic:server:resend-invoice", false, player, invoiceId)
  cb(success)
end)

RegisterNUICallback("delete-invoice", function(data, cb)
  local invoiceId = data.invoiceId
  local success = lib.callback.await("DERP-mechanic:server:delete-invoice", false, invoiceId)
  cb(success)
end)
...
RegisterNUICallback("pay-invoice", function(data, cb)
  ...
  local success = lib.callback.await("DERP-mechanic:server:pay-invoice", false, activeInvoiceData.invoiceId, activeInvoiceData.senderPlayerId, paymentMethod)
...
RegisterNetEvent("DERP-mechanic:client:show-invoice-to-player", function(senderPlayerId, invoiceId, invoiceItems, invoiceTotal)
  activeInvoiceData = {
    invoiceId = invoiceId,
