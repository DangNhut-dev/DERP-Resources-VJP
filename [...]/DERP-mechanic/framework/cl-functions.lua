...
RegisterNetEvent("DERP-mechanic:client:notify", function(...)
...
function Framework.Client.GetVehicleValue(model)
  ...
  if GetResourceState("jg-dealerships") == "started" then
    local price = lib.callback.await("DERP-mechanic:server:dealerships-vehicle-value", false, hash)
...
function Framework.Client.GetPlate(vehicle)
  ...
  if GetResourceState("brazzers-fakeplates") == "started" then
    local originalPlate = lib.callback.await("DERP-mechanic:server:brazzers-get-plate-from-fakeplate", false, plate)
...
function Framework.Client.GetPlayerJobDuty(mechanicId)
  ...
  local onDuty = lib.callback.await("DERP-mechanic:server:is-on-duty", false, mechanicId)
...
function Framework.Client.ToggleJobDuty(toggle)
  lib.callback.await("DERP-mechanic:server:toggle-duty", false, toggle)
end
...
function Framework.Client.OpenInventoryStash(stashId, stashName)
  ...
  elseif (Config.Inventory == "auto" and GetResourceState("qb-inventory") == "started") or Config.Inventory == "qb-inventory" then
    TriggerServerEvent("DERP-mechanic:server:open-inventory-stash", stashId, stashName)
