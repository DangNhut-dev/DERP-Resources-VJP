...
function Framework.Server.Notify(src, msg, type)
  TriggerClientEvent("DERP-mechanic:client:notify", src, msg, type, 5000)
end
...
lib.callback.register("DERP-mechanic:server:is-admin", function(src)
...
if GetResourceState("brazzers-fakeplates") == "started" then
  lib.callback.register("DERP-mechanic:server:brazzers-get-plate-from-fakeplate", function(_, fakeplate)
...
lib.callback.register("DERP-mechanic:server:has-item", Framework.Server.HasItem)
...
RegisterNetEvent("DERP-mechanic:server:remove-item", function(...)
...
lib.callback.register("DERP-mechanic:server:toggle-duty", function(src, toggle)
...
lib.callback.register("DERP-mechanic:server:get-society-balance", function(_, society, type)
...
RegisterServerEvent("DERP-mechanic:server:open-inventory-stash", function(stashId, stashName)
  local src = source
...
lib.callback.register("DERP-mechanic:server:dealerships-vehicle-value", function(src, vehicleHash)
...
---@param plate string
---@param props table
lib.callback.register("DERP-mechanic:server:save-vehicle-props", function(src, plate, props)
