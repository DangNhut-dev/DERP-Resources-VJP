...
-- Sửa lại onVehicleAttemptToEnter (fix bug doorslockstate truthy + skip xe player)
local function onVehicleAttemptToEnter(vehicle)
    if Entity(vehicle).state.doorslockstate ~= nil then return end
    if Entity(vehicle).state.owner then return end
...
local function lockNearbyNPCVehicles()
    local vehicles = GetGamePool('CVehicle')
    for i = 1, #vehicles do
        local veh = vehicles[i]
        if Entity(veh).state.doorslockstate ~= nil then goto continue end
        if Entity(veh).state.owner then goto continue end

        local driverPed = GetPedInVehicleSeat(veh, -1)
        if IsPedAPlayer(driverPed) then goto continue end

        local isParked = driverPed == 0
        if not isParked then goto continue end

        local vehicleConfig = GetVehicleConfig(veh)
        if vehicleConfig.noLock then goto continue end
