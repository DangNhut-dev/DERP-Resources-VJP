...
function SendPoliceAlertAttempt(crime, vehicle)
    ...
    local hoursOffset = (24 + GetClockHours() - config.policeAlertNightStartHour) % 24
...
local islockpickingProcessLocked = false
---Lockpicking minigame.
---@param isAdvancedLockedpick boolean Determines whether an advanced lockpick was used
---@param maxDistance number? The max distance to check.
---@param customChallenge boolean? lockpick challenge override
function LockpickDoor(isAdvancedLockedpick, maxDistance, customChallenge)
    ...
    CreateThread(function()
        ...
        local isSuccess = customChallenge or exports['t3_lockpick']:startLockpick(isAdvancedLockedpick and 'advancedlockpick' or 'lockpick')

        if getIsVehicleInRange(vehicle, maxDistance) then
...
---Will be executed when the hotwire is successful.
---@param vehicle number The entity number of the vehicle.
local function hotwireSuccessCallback(vehicle)
    TriggerServerEvent('qbx_vehiclekeys:server:hotwiredVehicle', VehToNet(vehicle))
end

---Operations done after the Hotwire minigame done.
---@param vehicle number The entity number of the vehicle.
---@param isAdvancedLockedpick boolean Determines whether an advanced lockpick was used.
---@param isSuccess boolean? Determines whether the hotwire was successful.
...
local isHotwiringProcessLocked = false
---Hotwiring with a tool minigame.
---@param vehicle number The entity number of the vehicle.
---@param isAdvancedLockedpick boolean Determines whether an advanced lockpick was used
---@param customChallenge boolean? lockpick challenge override
function Hotwire(vehicle, isAdvancedLockedpick, customChallenge)
    ...
    if isHotwiringProcessLocked then return end
    isHotwiringProcessLocked = true
    ...
    CreateThread(function()
        ...
        isHotwiringProcessLocked = false
    end)
end