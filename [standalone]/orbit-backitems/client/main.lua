...
local props = {
	-- ["weapon_g26"]  = { model = "w_pi_g26", hash = joaat("weapon_g26"), tier = 5 },


	-- ["wetbud"] = { model = "bkr_prop_weed_drying_02a", hash = joaat("bkr_prop_weed_drying_02a"), tier = 1, yr = 90.0 },
	...
	-- ["markedbills"] = { model = "prop_money_bag_01", hash = joaat("prop_money_bag_01"), tier = 3, x = -0.47, zr = 0 },
	...
	["weapon_dracocustom"]   = { model = "w_ar_dracocustom", hash = joaat("weapon_dracocustom"), tier = 1},
	["weapon_ar15full"]   = { model = "w_ar_ar15full_default", hash = joaat("weapon_ar15full"), tier = 1},
	["weapon_ak74"]          = { model = "w_ar_ak74", hash = joaat("weapon_ak74"), tier = 1 },
	["weapon_mp5"]           = { model = "w_sb_mp5", hash = joaat("weapon_mp5"), tier = 1 },
	["weapon_hk416"]         = { model = "w_ar_hk416", hash = joaat("weapon_hk416"), tier = 1, z = 0.15, x = 0.1, y = -0.18 },
	["weapon_mp9"]           = { model = "w_sb_mp9", hash = joaat("weapon_mp9"), tier =  1, yr = 90.15, y = -0.15, z = -0.06, x = -0.06},
	["weapon_uzi"]           = { model = "w_sb_uzi", hash = joaat("weapon_uzi"), tier = 1 },
	["weapon_scarh"]         = { model = "w_ar_scarh", hash = joaat("weapon_scarh"), tier = 1 },
	["weapon_remington"]     = { model = "w_sg_remington", hash = joaat("weapon_remington"), tier = 1, y = -0.18, z = 0.0 },
	...
	["weapon_m110"]           = { model = "w_sr_m110", hash = joaat("weapon_m110"), tier = 1, y = -0.18, z = 0.0 },
	...
	-- ["weapon_katana"]    = { model = "w_me_katana", hash = joaat("weapon_katana"), tier = 2, zr = -90.0, xr = -40.0, y = -0.14, x = 0.2, z = -0.08 },
...
local function calcOffsets(x, y, z, xr, yr, zr, item)
	...
	if props[item].x then X = props[item].x end
	if props[item].y then Y = props[item].y end
	if props[item].z then Z = props[item].z end
	if props[item].xr then XR = props[item].xr end
	if props[item].yr then YR = props[item].yr end
	if props[item].zr then ZR = props[item].zr end
...
local function DeleteWeapon(item)
	...
	while GetSelectedPedWeapon(ped) ~= hash do
		Wait(100)
		wait = wait + 1
		if wait >= 30 then WeapDelete = false return end
	end
			...
			if item then
				if tempBox ~= item then
					...
					if not hasitem or (props[item] and props[item].busy) then
						...
						if v.chain then carryingChain = nil end
						if v.carry then carryingBox = nil end
...
local function DoItemCheck()
	...
	if IsPedShooting(PlayerPedId()) or IsPlayerFreeAiming(PlayerId) then return end
	doingCheck = true
...
local function toggleProps()
	if override then
		if items_attatched then
			for k, v in pairs(items_attatched) do
				DeleteObject(v.handle)
				if v.slot then PlayerSlots[v.tier][v.slot].isBusy = false end
				if v.carry then carryingBox = nil end
				if v.chain then carryingChain = nil end
				items_attatched[k] = nil
			end
...
RegisterNetEvent('weapons:client:SetCurrentWeapon', function(data)
	...
	elseif not data and LocalPlayer.state.isLoggedIn then
		Wait(1000)
		if not override then DoItemCheck() end
	end
end)
...
local pedCache = cache.ped
CreateThread(function()
    while true do
        Wait(500)
        if cache.ped ~= pedCache then
            pedCache = cache.ped
            Wait(1000)
            if not override then refreshProps() end
        end
    end
end)