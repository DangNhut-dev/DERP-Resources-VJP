...
RegisterServerCallback('qbox:getPlayerData', function(source)
    DebugPrint("QBox getPlayerData called for source: " .. source)
    local player = exports.qbx_core:GetPlayer(source)
    if not player then
        print("[ERROR] QBox Player not found for source: " .. source)
        return {}
    end
    ...
    data.cash = MySQL.scalar.await('SELECT coin FROM derp_coin WHERE citizenid = ?', { citizenid }) or 0
    data.bank = money.bank
    data.playTime = FormatPlayTime(playerTimes[citizenid] or 0)
    data.lang = Locales[CFG.Locale] or Locales['en']
    DebugPrint("QBox data prepared for " .. citizenid .. " - PlayTime: " .. (playerTimes[citizenid] or 0))
    return data
end)

MySQL.ready(function()
    DebugPrint("QBox MySQL ready - starting playtime loading...")
    MySQL.Async.fetchAll('SELECT citizenid, playtime FROM players', {}, function(result)
        if result and #result > 0 then
            DebugPrint("QBox results from database: " .. #result)
            for i = 1, #result, 1 do
                ...
                DebugPrint("QBox loaded " .. citizenid .. ": " .. playtime .. " seconds")
            end
            if CFG.Debug then
                DebugPrint("QBox loaded " .. #result .. " playtime from database")
            end
        else
            print("[WARNING] QBox no playtime data found in database or query error")
        end
    end)
end)
...
AddEventHandler('QBCore:Server:PlayerLoaded', function(player)
    ...
    DebugPrint("QBox PlayerLoaded: " .. citizenid .. " playtime: " .. playerTimes[citizenid])
end)
...
AddEventHandler('QBCore:Server:OnPlayerUnload', function(source)
    ...
    if player and player.PlayerData then
        ...
        lastSaved[citizenid] = playtime
        DebugPrint("QBox OnPlayerUnload: Saved " .. citizenid .. " playtime: " .. playtime)
    else
    ...
    if player and player.PlayerData then
        ...
        MySQL.Async.execute('UPDATE players SET playtime = @playtime WHERE citizenid = @citizenid', {
            ...
            if rowsChanged > 0 then
                DebugPrint("QBox playtime saved for " .. citizenid .. ": " .. playtime)
            end
        end)
        lastSaved[citizenid] = playtime
    else
        print("[WARNING] QBox player not found " .. src .. " in playerDropped")
    end
end)
    ...
    while true do
        ...
        for i = 1, #players do
            ...
            if player and player.PlayerData then
                ...
                DebugPrint("QBox playtime incremented for " .. citizenid .. ": " .. playerTimes[citizenid])
            end
...
AddEventHandler('onResourceStop', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        DebugPrint('QBox script stopping, saving all playtime...')
        local count = 0
        for citizenid, playtime in pairs(playerTimes) do
            ...
            count = count + 1
            DebugPrint("QBox saved " .. citizenid .. ": " .. playtime .. " seconds")
        end
        DebugPrint('QBox saved ' .. count .. ' playtime records before stop')
    end
end)
...
CreateThread(function()
    ...
    while true do
        ...
        local total = #idList
        if total == 0 then
            if CFG.Debug then DebugPrint('QBox auto-save skipped (no online users)') end
            goto continue
        end
        ...
        for idx = rotateIndex, endIndex do
            ...
            if lastSaved[citizenid] ~= playtime then
                ...
                wrote = wrote + 1
                yielded = yielded + 1
                DebugPrint("QBox auto-save: Saved " .. citizenid .. " playtime: " .. playtime)
                if (yielded % CHUNK_YIELD) == 0 then Wait(0) end
            end
        end
        if CFG.Debug then
            DebugPrint(('QBox auto-save chunk %d-%d/%d, wrote %d'):format(rotateIndex, endIndex, total, wrote))
        end
...
RegisterCommand('checkplaytime', function(source)
    ...
    if player then
        ...
        MySQL.Async.fetchScalar('SELECT playtime FROM players WHERE citizenid = @citizenid', {
            ...
            DebugPrint("QBox Player " .. citizenid .. ":")
