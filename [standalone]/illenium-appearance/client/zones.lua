...
local function RemoveZones()
    ...
    for i = 1, #Zones.CustomPedRoom do
        Zones.CustomPedRoom[i]:remove()
    end
end
...
local function IsCustomPed()
    return not FreemodeModels[GetEntityModel(cache.ped)]
end

local function onCustomPedRoomEnter(data)
    if not IsCustomPed() then return end
    local index = lookupZoneIndexFromID(Zones.CustomPedRoom, data.id)
    currentZone = {
        name = 'customPedRoom',
        index = index
    }
    local prefix = Config.UseRadialMenu and '' or '[E] '
    lib.showTextUI(
        prefix .. string.format(_L('textUI.clothing'), Config.ClothingCost),
        Config.TextUIOptions
    )
end

local function SetupCustomPedRoomZones()
    for _, v in pairs(Config.CustomPedRooms or {}) do
        Zones.CustomPedRoom[#Zones.CustomPedRoom + 1] = SetupZone(v, onCustomPedRoomEnter, onZoneExit)
    end
end
    ...
    while true do
        ...
        if currentZone then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                elseif currentZone.name == "surgeon" then
                    OpenSurgeonShop()
                elseif currentZone.name == 'customPedRoom' then
                    TriggerEvent('illenium-appearance:client:openClothingShopMenu')
                end
            end
        end
