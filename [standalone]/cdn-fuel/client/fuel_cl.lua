...
if Config.ShowNearestGasStationOnly then
	...
	CreateThread(function()
		...
		while true do
			...
			local location = 0
			local label = "Trạm Xăng" -- Prevent nil just in case, set default name.
	...
	if Config.Ox.Input then
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
			if ReserveLevels < maxfuel then
				...
				fuel = lib.inputDialog('Trạm xăng', {
					{ type = "input", label = 'Giá xăng', default = '$'.. FuelPrice .. ' Per Liter', disabled = true },
					{ type = "input", label = 'Nhiên liệu hiện tại', default = finalfuel .. ' Per Liter', disabled = true },
					{ type = "input", label = 'Đổ đầy bình xăng', default = maxfuel .. 'Per Liter', disabled = true },
					{ type = "input", label = 'Các trạm xăng hiện có', default = ReserveLevels, disabled = true },
					{ type = "slider", label = 'Giá đầy bình: $' ..wholetankcostwithtax.. '',default = ReserveLevels, min = 0, max = ReserveLevels},
				})
				...
				fuel = lib.inputDialog('Trạm xăng', {
					{ type = "input", label = 'Giá xăng', default = '$'.. FuelPrice .. ' Per Liter', disabled = true },
					{ type = "input", label = 'Nhiên liệu hiện tại', default = finalfuel .. ' Per Liter', disabled = true },
					{ type = "input", label = 'Đổ đầy bình xăng', default = maxfuel, disabled = true },
					{ type = "slider", label = 'Giá đầy bình: $' ..wholetankcostwithtax.. '', default = maxfuel, min = 0, max = maxfuel },
				})
