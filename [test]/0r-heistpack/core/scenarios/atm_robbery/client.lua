...
local robberyZones = {
    {
        coords = vector3(20.79, -1104.79, 42.37), -- thay coords thực tế
        radius = 2500.0,
    },
    -- Thêm zone khác nếu cần:
    -- {
    --     coords = vector3(500.0, -800.0, 25.0),
    --     radius = 200.0,
    -- },
}

local function isInRobberyZone()
...
function AtmRobberyClient.startExplodeAtm(model, entity)
    if not isInRobberyZone() then 
        Utils.notify(locale("atm_robbery.not_in_zone") or "You are not in the robbery zone", "error", 3000)
        return
    end
...
function AtmRobberyClient.startRopeAtm(model, entity)
    if not isInRobberyZone() then         
        Utils.notify(locale("atm_robbery.not_in_zone") or "Bạn không trong vùng cho phép", "error", 3000)
        return
    end
    local distance = #(GetEntityCoords(cache.ped) - GetEntityCoords(entity))
    if distance > 2.0 then
