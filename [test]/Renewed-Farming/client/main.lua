...
local function harvestSeeds(farm, payload)
    ...
    for i = 1, #payload do
        ...
        if Farms.isSpotUsed(farm, plant.index) then
            local success = plant.dist < 1.4 or moveToCoords(vec3(plant.coords.x, plant.coords.y, plant.coords.z + 0.1))


            if success then
                TaskTurnPedToFaceCoord(cache.ped, plant.coords.x, plant.coords.y, plant.coords.z + 0.1, 750)
                Wait(750)
...
local function placeSeed(data)
    ...
    while planting do
        ...
        if hit and entity and material == -1942898710 and coords then
            ...
            if spots and amount > 0 then
                ...
                if ePressed then
                    if not Farms.canInteract(currentFarm) then
                        lib.notify({ type = 'error', description = locale('not_your_plot') })
                        break
                    end
...
local function getPlants()
    ...
    while pitchForking do
        ...
        if entity ~= currentEntity or not currentFarm then
            currentEntity = entity
            currentFarm = material == -1942898710 and Farms.getFarm(GetEntityCoords(entity))
        end
...
local function harvestPlants()
    ...
    if not Farms.canInteract(farm) then
        return lib.notify({ type = 'error', description = locale('not_your_plot') })
    end
...
CreateThread(function()
    ...
    for k, v in pairs(farms) do
        Farms.addFarm(k, v.coords, v.plants, v.owner)
...
registerNetEvent('Renewed-Farming:client:updateFarmOwner', function(id, owner)
    Farms.updateOwner(id, owner)
end)

registerNetEvent('Renewed-Farming:client:addFarm', function(id, coords, points, owner)
    Farms.addFarm(id, coords, points, owner)
end)
