...
local filmKeyPressed = false

RegisterKeyMapping('qbx_newsjob:filmkey', 'Toggle Film Overlay', 'keyboard', 'k')
RegisterCommand('qbx_newsjob:filmkey', function()
    if holdingCam then
        filmKeyPressed = true
    end
end, false)
...
CreateThread(function()
    RegisterFontFile('fonts/Akrobat-Regular')
    fontId = RegisterFontId('Akrobat-Regular')
    AddTextEntry('WN_MSG',    "<FONT FACE='Akrobat-Regular'>~a~</FONT>")
    AddTextEntry('WN_BOTTOM', "<FONT FACE='Akrobat-Regular'>~a~</FONT>")
...
RegisterNetEvent('qbx_newsjob:client:toggleCam', function()
    if not holdingCam then
        local input = lib.inputDialog(locale('info.newscam_dialog_title'), {
            {
                type = 'input',
                label = locale('info.newscam_input_msg'),
                required = true,
                max = 80,
            },
            {
                type = 'input',
                label = locale('info.newscam_input_bottom'),
                required = false,
                max = 80,
            },
            {
                type = 'input',
                label = locale('info.newscam_input_title'),
                required = false,
                max = 120,
            }
        })
		...
		if QBX.PlayerData.job.name == 'lsnews' then
			if holdingCam then
				if IsControlJustReleased(1, 38) then
					...
					BeginTextCommandScaleformString('WN_MSG')
					AddTextComponentSubstringPlayerName(msg)
					EndTextCommandScaleformString()
					BeginTextCommandScaleformString('WN_BOTTOM')
					...
					PushScaleformMovieFunctionParameterInt(0)
					BeginTextCommandScaleformString('WN_TITLE')
