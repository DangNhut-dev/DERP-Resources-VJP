...
if FrameworkSelected == 'ESX' then
    MySQL.ready(function()
        ...
        Wait(1000)
        local length = tonumber(MySQL.query.await("SELECT COLUMN_NAME, CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'derp-multicharacter_slots' AND COLUMN_NAME = 'identifier' AND DATA_TYPE = 'varchar'")[1].CHARACTER_MAXIMUM_LENGTH)
        assert(length == 255, '^5Invalid VARCHAR length for table derp-multicharacter_slots!\n^2Please refer to the docs: https://zsx-development.gitbook.io/docs/multicharacter/common-issues/database\n\n')
    end)
end
...
Characters.GetSlots = function(src)
	...
	local amount =
		MySQL.prepare.await("SELECT `amount` FROM `derp-multicharacter_slots` WHERE `identifier` = ?", { identifier })

	if override then
		-- Override present in config
		if not amount then
			MySQL.insert.await(
				"INSERT INTO `derp-multicharacter_slots` (`identifier`, `amount`) VALUES (?, ?)",
				{ identifier, override }
			)
		elseif amount ~= override then
			MySQL.update.await(
				"UPDATE `derp-multicharacter_slots` SET `amount` = ? WHERE `identifier` = ?",
				{ override, identifier }
			)
		end
		amount = override
...
Characters.ConvertSkin = function(convertedIdentifier)
    ...
    if appearanceResource == 'skinchanger' then
        ...
        elseif FrameworkSelected == 'QBCore' then
            local result = MySQL.query.await("SELECT * FROM `playerskins` WHERE `citizenid` = ? AND active = 1 ORDER BY `id` DESC LIMIT 1", {convertedIdentifier})
            if result and result[1] and result[1].skin then
                skin = json.decode(result[1].skin)
            end
        end
...
Framework.RegisterServerCallback("DERP-multicharacter:CheckNameAvailability", function(source, cb, firstName, lastName)
...
Framework.RegisterServerCallback('DERP-multicharacter:IsPlayerActive', function(source, cb)
...
Framework.RegisterServerCallback('DERP-multicharacter:Get:Characters', function(source, cb)
    cb(Characters.Get(source))
end)

Framework.RegisterServerCallback('DERP-multicharacter:Get:FirstChar', function(source, cb)
    cb(Characters.GetFirst(source))
end)

Framework.RegisterServerCallback('DERP-multicharacter:Get:NumChar', function(source, cb, num)
    cb(Characters.GetNum(source, num))
end)

Framework.RegisterServerCallback('DERP-multicharacter:Get:PlayerSlots', function(source, cb)
    cb(Characters.GetSlots(source))
end)

Framework.RegisterServerCallback('DERP-multicharacter:Get:CharactersExists', function(source, cb)
    cb(Characters.DoesUserHaveCharacters(source))
end)

Framework.RegisterServerCallback('DERP-multicharacter:Get:Properties', function(source, cb, id)
    cb(Characters.GetProperties(source, id))
end)

Framework.RegisterServerCallback('DERP-multicharacter:Event:SelectedCharacter', function(source, cb, id)
    ...
    elseif FrameworkSelected == 'QBCore' then
        ...
        if QBCore.Player.Login(source, response[1].citizenid) then
            ...
            TriggerClientEvent('DERP-multicharacter:CreateQBInstance', source)
            cb('DONE')
...
Framework.RegisterServerCallback('DERP-multicharacter:Event:RemoveCharacter', function(source, cb, id)
    local convertedIdentifier = Characters.Remove(source, id)
    cb(Characters.DoesUserHaveCharacters(source), convertedIdentifier)
end)

Framework.RegisterServerCallback('DERP-multicharacter:Event:Logout', function(source, cb)
    if FrameworkSelected == 'ESX' then
...
RegisterNetEvent('DERP-multicharacter:Save:Appereance')
AddEventHandler('DERP-multicharacter:Save:Appereance', function(skin)
    ...
    elseif FrameworkSelected == 'QBCore' then
        ...
        MySQL.insert.await('INSERT INTO playerskins (citizenid, model, skin, active) VALUES (?, ?, ?, ?)', { citizenid, skin.model, json.encode(skin), 1 })
    end
end)

Framework.RegisterServerCallback('DERP-multicharacter:Create:Player', function(source, cb, data)
    local isTimedOut = Player(source).state['ZSX_isPlayerTimedOut']
    if isTimedOut then
        if isTimedOut > GetGameTimer() then
...
function Logout(source)
    TriggerClientEvent('DERP-multicharacter:LogoutPlayer', source)
end
