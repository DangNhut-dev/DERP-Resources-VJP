...
Framework.SetSkin = function(skinData, isMale, model)
    ...
    if skinData == nil then
        local defaultClothes = isMale and Config.InitialPlayerClothes.Male or Config.InitialPlayerClothes.Female
        if defaultClothes then
            for _, comp in ipairs(defaultClothes.Components) do
                SetPedComponentVariation(ped, comp.component_id, comp.drawable, comp.texture, 0)
            end
            for _, prop in ipairs(defaultClothes.Props) do
            ...
            if defaultClothes.Hair then
                SetPedHairColor(ped, defaultClothes.Hair.color, defaultClothes.Hair.highlight)
            end
        end
        return
    end
...
Framework.OpenSkinMenu = function(gender)
    ...
    elseif Framework.AppereanceResource == 'fivem-appearance' then
        ...
        exports['fivem-appearance']:startPlayerCustomization(function (skin)
            if skin then
                TriggerServerEvent('DERP-multicharacter:Save:Appereance', skin)
            else
