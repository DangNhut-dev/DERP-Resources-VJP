...
function StartRefreshTimer(src, jobName)
    ...
    RefreshTimers[src] = SetTimeout(interval, function()
        ...
        if Player and Player.PlayerData.job.name == jobName then
            ...
            TriggerClientEvent('DERP-bossmenu:client:RefreshData', src)
...
-- Get user settings
QBCore.Functions.CreateCallback('DERP-bossmenu:server:GetSettings', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
-- Save user settings
RegisterNetEvent('DERP-bossmenu:server:SaveSettings', function(settings)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('QBCore:Server:OnJobUpdate', function(source, newJob)
    ...
    if Player then
        ...
        TriggerClientEvent('DERP-bossmenu:client:JobChanged', -1, newJob.name)
    end
end)
...
QBCore.Commands.Add('managejob', 'Open job management interface', {}, false, function(source)
    ...
    TriggerClientEvent('DERP-bossmenu:client:TriggerOpenManager', source, {jobData = jobName})
end)

RegisterNetEvent('DERP-bossmenu:server:RefreshPlayTime', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
function LoadSocietyTransactions()
    ...
    if result and #result > 0 then
        ...
        print("^2[DERP-bossmenu]^7 No society transactions found in database")
    end
end
...
QBCore.Functions.CreateCallback('DERP-bossmenu:server:GetSocietyData', function(source, cb, jobName)
    local src = source
...
RegisterNetEvent('DERP-bossmenu:server:DepositMoney', function(amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('DERP-bossmenu:server:WithdrawMoney', function(amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('DERP-bossmenu:server:TransferMoney', function(citizenid, amount, note, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('DERP-bossmenu:server:SubmitApplication', function(jobName, answers)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('DERP-bossmenu:server:GetJobApplications', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('DERP-bossmenu:server:CheckPendingApplication', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('DERP-bossmenu:server:CheckApplicationStatus', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('DERP-bossmenu:server:RequestRefreshJobData', function()
    TriggerEvent('QBCore:server:UpdateObject')
end)
...
QBCore.Functions.CreateCallback('DERP-bossmenu:server:GetEmployeePermissions', function(source, cb, citizenid, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('DERP-bossmenu:server:UpdateEmployeePermissions', function(source, cb, citizenid, jobName, permissions)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    MySQL.Async.execute('INSERT INTO job_employee_permissions (citizenid, job, permissions, granted_by) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE permissions = ?, granted_by = ?', 
        ...
        if rowsChanged > 0 then
            ...
            local targetPlayer = QBCore.Functions.GetPlayerByCitizenId(citizenid)
            if targetPlayer then
                TriggerClientEvent('DERP-bossmenu:client:RefreshPermissions', targetPlayer.PlayerData.source, permissions)
            ...
            for _, p in pairs(players) do
                if p.PlayerData.job.name == jobName then
                    TriggerClientEvent('DERP-bossmenu:client:SyncPermissions', p.PlayerData.source, citizenid, permissions)
                end
            end
...
RegisterServerEvent('DERP-bossmenu:server:PermissionAction', function(action, data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('DERP-bossmenu:server:GetPlaytimeData', function(source, cb, jobName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
