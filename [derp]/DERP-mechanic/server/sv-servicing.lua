...
lib.callback.register("DERP-mechanic:server:pay-for-service", function(source, plate, partName)
  ...
  local mileage = exports["DERP-vehiclemileage"]:GetMileage(plate)
...
lib.callback.register("DERP-mechanic:server:get-servicing-history", function(source, plate)
  local playerState = Player(source).state
  local mechanicId = playerState.mechanicId
  if not mechanicId then return false end
  local hasPermission = isEmployee(source, mechanicId, { "mechanic", "manager" }, true)
  ...
  local history = exports["DERP-mechanic"]:getVehicleServiceHistory(plate)
  local mileage, unit = exports["DERP-vehiclemileage"]:GetMileage(plate)
  return {
