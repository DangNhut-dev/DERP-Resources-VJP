...
RegisterNetEvent("DERP-mechanic:server:use-nitrous-bottle", function()
  local source = source
...
lib.callback.register("DERP-mechanic:server:can-refill-bottle-in-current-vehicle", function(source)
  local playerPed = GetPlayerPed(source)
  local vehicle = GetVehiclePedIsIn(playerPed, false)
...
lib.callback.register("DERP-mechanic:server:refill-nitrous-bottle", function(source)
...
lib.callback.register("DERP-mechanic:server:install-new-bottle", function(source)
  local playerState = Player(source).state
  if not playerState then
    return false
  end
