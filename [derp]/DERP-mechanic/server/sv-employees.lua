...
lib.callback.register("DERP-mechanic:server:is-mechanic-employee", function(source, mechanicId)
  return isEmployee(source, mechanicId, { "owner", "manager", "mechanic" }, true)
end)

RegisterNetEvent("DERP-mechanic:server:request-hire-employee", function(data)
  local source = source
  ...
  TriggerClientEvent("DERP-mechanic:client:show-confirm-employment", data.playerId, data)
end)

RegisterNetEvent("DERP-mechanic:server:employee-hire-rejected", function(requesterId)
  Framework.Server.Notify(requesterId, Locale.employeeRejectedMsg, "error")
end)

RegisterNetEvent("DERP-mechanic:server:hire-employee", function(data)
  local source = source
  ...
  TriggerClientEvent("DERP-mechanic:client:refresh-mechanic-zones-and-blips", source)
end)

RegisterNetEvent("DERP-mechanic:server:fire-employee", function(identifier, mechanicId)
  local source = source
  ...
  if targetPlayer then
    ...
    TriggerClientEvent("DERP-mechanic:client:refresh-mechanic-zones-and-blips", targetPlayer)
  else
...
RegisterNetEvent("DERP-mechanic:server:update-employee-role", function(identifier, mechanicId, newRole)
  local source = source
  local hasPermission = isEmployee(source, mechanicId, "manager", true)
  if not hasPermission then
    return Framework.Server.Notify(source, Locale.employeePermissionsError, "error")
