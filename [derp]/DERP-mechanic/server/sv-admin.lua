lib.addCommand(Config.MechanicAdminCommand or "mechanicadmin", {
  ...
  TriggerClientEvent("DERP-mechanic:client:open-admin", source)
end)

lib.callback.register("DERP-mechanic:server:get-admin-data", function(source)
  if not Framework.Server.IsAdmin(source) then
    Framework.Server.Notify(source, Locale.insufficientPermissions, "error")
    return false
  end
...
lib.callback.register("DERP-mechanic:server:delete-mechanic-data", function(source, mechanicId)
  if not Framework.Server.IsAdmin(source) then
    Framework.Server.Notify(source, Locale.insufficientPermissions, "error")
    return false
  end
...
lib.callback.register("DERP-mechanic:server:set-mechanic-owner", function(source, mechanicId, targetPlayerId)
  if not Framework.Server.IsAdmin(source) then
    Framework.Server.Notify(source, Locale.insufficientPermissions, "error")
    return false
  end
