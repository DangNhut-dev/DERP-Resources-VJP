...
lib.callback.register("DERP-mechanic:server:connect-vehicle", function(src, vehicleNetId, netId)
...
lib.callback.register("DERP-mechanic:server:disconnect-vehicle", function(src, vehicleNetId)
    local connection = activeTabletConnections[vehicleNetId]
...
lib.callback.register("DERP-mechanic:server:get-player-mechanics", function(src)
...
lib.callback.register("DERP-mechanic:server:get-tablet-mechanic-data", function(src, mechanicName)
...
lib.callback.register("DERP-mechanic:server:get-vehicle-mileage", function(src, vehicleNetId)
    return exports["DERP-vehiclemileage"]:GetMileage(vehicleNetId)
end)

lib.callback.register("DERP-mechanic:server:toggle-on-duty", function(src, onDuty)
...
lib.callback.register("DERP-mechanic:server:is-on-duty", function(src, mechanicName)
...
lib.callback.register("DERP-mechanic:server:get-tablet-preferences", function(src)
    local identifier = Framework.Server.GetPlayerIdentifier(src)
...
lib.callback.register("DERP-mechanic:server:save-tablet-settings", function(src, preferences)
    local playerState = Player(src).state
    if not playerState or not playerState.mechanicId then
        return false
    end
...
if Config.UseTabletCommand then
    lib.addCommand(Config.UseTabletCommand == true and "tablet" or Config.UseTabletCommand, {
        ...
        TriggerClientEvent("DERP-mechanic:client:use-tablet", source)
    end)
end