...
RegisterNUICallback("install-new-bottle", function(data, cb)
  ...
  playMinigame(connectedVehicle, "prop", { prop = "canister" }, function(success)
    ...
    local itemAdded = lib.callback.await("DERP-mechanic:server:install-new-bottle", false)
...
RegisterNUICallback("refill-bottle", function(data, cb)
  Framework.Client.ProgressBar(Locale.refillingBottle, 5000, false, false, function()
    cb(lib.callback.await("DERP-mechanic:server:refill-nitrous-bottle", false))
  end, function()
    cb(false)
  end)
end)

RegisterNetEvent("DERP-mechanic:client:use-nitrous-bottle")
AddEventHandler("DERP-mechanic:client:use-nitrous-bottle", function()
  local canRefill = lib.callback.await("DERP-mechanic:server:can-refill-bottle-in-current-vehicle", false)
  if not canRefill then
    return
  end

  Framework.Client.ProgressBar(Locale.refillingBottle, 2500, false, false, function()
    TriggerServerEvent("DERP-mechanic:server:use-nitrous-bottle")
  end, function()
