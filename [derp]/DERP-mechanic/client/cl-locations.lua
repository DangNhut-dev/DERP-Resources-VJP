...
function createMechanicZonesAndBlips()
  ...
  local mechanicsData = lib.callback.await("DERP-mechanic:server:get-mechanic-locations-data", false)
  ...
  for _, mechanicData in pairs(mechanicsData) do
    ...
    local isMechanicEmployee = lib.callback.await("DERP-mechanic:server:is-mechanic-employee", false, name)
  ...
  while not lifts and attempts < maxAttempts do
    lifts = lib.callback.await("DERP-mechanic:server:get-created-lifts")
    attempts += 1
    Wait(2500)
  end
  ...
  for mechanicId, mechanicLifts in pairs(lifts) do
    if mechanicLifts and Config.MechanicLocations[mechanicId] then
      ...
      local isMechanicEmployee = lib.callback.await("DERP-mechanic:server:is-mechanic-employee", false, mechanicId)
...
RegisterNetEvent("DERP-mechanic:client:refresh-mechanic-zones-and-blips", function()
  Wait(1000)
