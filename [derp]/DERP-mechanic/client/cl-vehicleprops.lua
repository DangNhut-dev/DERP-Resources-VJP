local function retrieveAndApplyVehicleStatebagData(vehicle, plate)
  ...
  lib.callback.await("DERP-mechanic:server:retrieve-and-apply-veh-statebag-data", false, VehToNet(vehicle), plate)
end
...
local function onEnterVehicle(vehicle)
  if vehiclePlate then
    lib.callback.await("DERP-mechanic:server:save-veh-statebag-data-to-db", false, vehiclePlate, true)
  end
