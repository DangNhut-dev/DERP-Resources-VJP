function setVehicleStatebag(vehicle, key, value, saveToDb, plate)
  return lib.callback.await("DERP-mechanic:server:set-vehicle-statebag", false, VehToNet(vehicle), key, value, saveToDb, plate)
end

function setVehicleStatebags(vehicle, data, saveToDb, plate)
  return lib.callback.await("DERP-mechanic:server:set-vehicle-statebags", false, VehToNet(vehicle), data, saveToDb, plate)
end
...
RegisterNUICallback("has-item", function(data, cb)
  cb(lib.callback.await("DERP-mechanic:server:has-item", false, data.itemName, data.qty))
end)

RegisterNUICallback("nearby-players", function(data, cb)
  local coords = GetEntityCoords(cache.ped)
  local radius = 10.0
  cb(lib.callback.await("DERP-mechanic:server:nearby-players", false, coords, radius, data and data.includePlayer or false))
end)
