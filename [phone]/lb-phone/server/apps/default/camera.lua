-- Camera app server-side handlers
...
-- Media type mapping for API requests
local mediaTypes = {
    Audio = "audio",
    Image = "image", 
    Video = "video"
...
-- Valid metadata types for photos/videos
local validMetadata = {
    selfie = true,
    import = true,
    screenshot = true
...
-- Get base URL for media uploads
...
-- Handle voice recording peer listening
...
-- Handle player disconnect for voice recording
...
-- Get upload API key for media uploads
...
-- Notify album members about updates
...
-- Check if phone number has access to album
...
-- Update album and notify members
...
-- Delete photos/videos from gallery
...
-- Toggle favourite status for photos/videos
...
-- Get images/videos with filtering
BaseCallback("camera:getImages", function(source, phoneNumber, filters, page)
    ...
    if filters.album then
        ...
        -- Remove phone_number condition for album queries
...
-- Media type categories for homepage
...
-- Get homepage data with albums and media counts
BaseCallback("camera:getHomePageData", function(source, phoneNumber)
    ...
    -- Adjust duplicate counts (add 1 if duplicates exist)
    ...
    -- Ensure counts are numbers for all albums
    for i = 1, #albums do
...
-- Remove member from album (callback)
...
BaseCallback("camera:toggleAlbumSharing", function(source, phoneNumber, albumId, shared)
    ...
    if success and not shared then
        -- Remove all members if making album private
