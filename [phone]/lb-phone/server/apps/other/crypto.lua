-- Crypto app server-side functionality
-- Handles cryptocurrency trading, transfers, and portfolio management
...
-- Function to make HTTP requests to CoinGecko API with rate limiting
local function MakeAPIRequest(endpoint)
    ...
    PerformHttpRequest("https://api.coingecko.com/api/v3/" .. endpoint, function(statusCode, responseData)
        local data = false
...
-- Background thread to fetch coin data periodically
CreateThread(function()
    while true do
        FetchCoinData()
        cryptoData.hasFetched = true
        
        -- Notify all clients of updated coin data
...
-- Helper function to add crypto to user's portfolio
...
-- Callback to get user's crypto portfolio
RegisterCallback("crypto:get", function(source)
    local identifier = GetIdentifier(source)
    
    -- Wait for data to be fetched and database to be ready
    ...
    -- Clone coin data and add user's holdings
...
-- Callback to buy cryptocurrency
...
-- Callback to sell cryptocurrency
...
-- Callback to transfer cryptocurrency
BaseCallback("crypto:transfer", function(source, phoneNumber, coinId, amount, targetNumber)
    ...
    if targetSource then
        ...
        if not Config.Item.Unique then
            ...
            targetIdentifier = MySQL.scalar.await("SELECT owned_id FROM phone_phones WHERE phone_number = ?", {
                targetNumber
            })
        end
    end
    ...
    -- Update client if target is online
    if targetSource then
...
-- Export function to add crypto to a player
...
-- Export function to remove crypto from a player
...
-- Export function to add custom coins
...
-- Export function to get coin data
