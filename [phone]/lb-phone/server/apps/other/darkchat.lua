-- DarkChat app server-side functionality
-- Handles anonymous chat channels, user authentication, and messaging

-- Helper function to notify all users with a specific username
...
-- Callback to set password for DarkChat account
...
-- Callback to register new DarkChat account
...
CreateAuthenticatedCallback("changePassword", function(source, phoneNumber, username, oldPassword, newPassword)
    ...
    -- Notify other devices and log them out
    ...
    -- Remove other logged in sessions
...
CreateAuthenticatedCallback("joinChannel", function(source, phoneNumber, username, channelName)
    ...
    -- If new channel, return basic info
    if not channelExists then
        return {
            name = channelName,
            members = 1
...
-- Export function to send DarkChat message
...
-- Export function to send DarkChat location
