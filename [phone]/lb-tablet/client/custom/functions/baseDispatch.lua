...
local function GetNearbyPedForDispatch(coords, customCheck)
    ...
    for i = 1, #peds do
        ...
        local blocked = nil
        if populationType ~= 4 and populationType ~= 5 then blocked = 'popType:'..populationType
        elseif IsPedDeadOrDying(ped, true) then blocked = 'dead'
        elseif pedType ~= 4 and pedType ~= 5 then blocked = 'pedType:'..pedType
        elseif distance > Config.BaseDispatch.MaxDistance then blocked = 'tooFar:'..math.floor(distance)
        elseif IsEntityAMissionEntity(ped) then blocked = 'mission'
        elseif customCheck and not customCheck(ped, pedCoords, distance) then blocked = 'customCheck:dist='..math.floor(distance)
        end
