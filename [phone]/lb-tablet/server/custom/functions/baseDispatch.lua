...
-- Cache config vì Config.BaseDispatch có thể bị nil sau khi load
local BaseDispatchConfig = Config.BaseDispatch
local BaseDispatchActions = Config.BaseDispatch.Actions
...
local function IsDispatchOnCooldown(action)
    if not BaseDispatchConfig or not BaseDispatchActions then return false end
    local actionData = BaseDispatchActions[action]
...
RegisterNetEvent("lb-tablet:addBaseDispatch", function(action, data)
    if not BaseDispatchConfig or not BaseDispatchActions then return end
    if not action or not data then return end
    if not BaseDispatchActions[action] then return end

    if IsDispatchOnCooldown(action) then return end
...
local explosionOptions = Config.BaseDispatch and Config.BaseDispatch.Actions and Config.BaseDispatch.Actions.Explosion
