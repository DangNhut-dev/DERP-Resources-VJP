...
Config.RequireGear = false                              -- Đặt false để cho phép vào job mà không cần trang bị gear
...
Config.GiveOnlyOneItemFromTable = true                 -- Đặt true nếu mỗi lần đập tường chỉ nhận 1 item từ Config.ItemsWhileMining. Nếu true thì nên sắp xếp item từ hiếm → dễ

Config.ItemsWhileMining = {                             -- Danh sách item có thể nhận mỗi lần đập tường
    {
        chance = 10,                                   -- Tỉ lệ nhận item (0-100). 1 = 1%, 100 = 100%
        itemName = "stone",                           -- Tên item
        quantity = function(isOnBonus, src)            -- Bắt buộc là function
            return math.random(1, 1)                   -- Phải trả về số lượng. Ví dụ random từ 1 đến 5
        end,
    },
}
...
Config.Lang = {
    ...
    ["endJobHint"] = "Nhấn ~b~[E]~s~ để ~y~kết thúc~s~ công việc.",
    ["openDoors"] = "Nhấn ~b~[E]~s~ để mở cửa",
    ["goDown"] = "Nhấn ~b~[E]~s~ để xuống hầm mỏ",
    ["goBack"] = "Nhấn ~b~[E]~s~ để quay lại và kết thúc công việc",
    ["startMining"] = "~b~[E] |~s~ Bắt Đầu Đào",
    ["mouseForMine"] = "Giữ ~b~[Trái Chuột]~s~ để đào",
    ...
    ["pickUp"] = "Nhấn ~b~[E]~s~ để nhặt",
    ["takeRails"] = "Đến kho lấy đường ray",
    ["placeProp"] = "Nhấn ~b~[E]~s~ để đặt",
    ["placePropBack"] = "Nhấn ~b~[E]~s~ để cất vật phẩm",
    ...
    ["forceExit"] = "Nhấn ~b~[E]~s~ để thoát hầm mỏ",
    ...
    ["lightToggle"] = "Nhấn ~b~[H]~s~ để bật/tắt đèn",
...
Config.Events = {
    ["gas"] = {
        chance = 3,               -- Tỉ lệ kích hoạt sự kiện (có thể xảy ra mỗi lần đập). Giá trị từ 0-100
        duration = 30 * 1000,        -- Thời gian sự kiện kéo dài (milliseconds)
        minimumProgressPercent = 30, -- % tiến độ tối thiểu của job để sự kiện có thể xảy ra
    ...
    ["blackout"] = {
        chance = 10,                -- Tỉ lệ kích hoạt sự kiện (có thể xảy ra mỗi lần đập). Giá trị từ 0-100
        minimumProgressPercent = 5, -- % tiến độ tối thiểu của job để sự kiện có thể xảy ra
