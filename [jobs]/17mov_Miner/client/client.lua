...
function CreateNewElevator()
    ...
    function elevatorData.animateDoors(isBack, param2)
        ...
        if isBack then
            if not elevatorData.backDoorsOpened and param2 then
                return Functions.Error("TRIED TO CLOSE BACK DOORS WHILE THEY'RE ALREADY CLOSED")
            end
        end
        ...
        if not isBack then
            ...
            elevatorData.backDoorsOpened = not param2
            ...
            if carriedObject then
                ...
                if propsObjectsTable ~= nil then
                    for propType, propList in pairs(propsObjectsTable) do
                        ...
                        if carriedPropType == propType and distance < 5 then
                ...
                for i = 1, #allProps do
                    if carriedObject then
                        ...
                        local slotCategory = nil
                        local slotSubtype = nil
                        for cat, catList in pairs(propsObjects) do
                            for _, ent in pairs(catList) do
                                if ent == allProps[i] then
                                    slotCategory = cat
                                    if cat == "supports" then
                                        slotSubtype = propsObjectsEntitySubtype[ent]
                                    end
                                end
                            end
                        end

                        local carriedCategory = nil
                        local carriedSubtype = nil
                        if carriedPropType == "Rails" then
                            carriedCategory = "rails"
                        elseif carriedPropType == "Lights" then
                            carriedCategory = "lights"
                        elseif carriedPropType ~= nil then
                            carriedCategory = "supports"
                            carriedSubtype = carriedPropType
                        end

                        local canPlace = false
                        if carriedCategory ~= nil and slotCategory == carriedCategory then
                            if carriedCategory == "supports" then
                                canPlace = (carriedSubtype == slotSubtype)
                            else
                                canPlace = true
                            end
                        end
                ...
                if closestProp and closestDistance < 5 and closestPropType then
                    ...
                    if closestDistance < propConfig.interactionDistance then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            carriedPropType = closestPropType
      ...
      if supportObj then
        if DoesEntityExist(supportObj) then
          ...
          if dotProduct > 1.5 then
            ...
            propsObjectsEntitySubtype[supportObj] = partType
...
local hitCounter = 0
local carriedPropType = nil
local propsObjectsEntitySubtype = {}
