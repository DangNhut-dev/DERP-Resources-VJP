...
RegisterNetEvent('delivery:server:registerVehicle', function(netId)
    local src = source
    if not activeJobs[src] then return end
    if type(netId) ~= 'number' then return end
    activeJobs[src].vehicleNetId = netId
end)
...
RegisterNetEvent('delivery:server:arrivedPoint', function(pointIndex)
    ...
    if not job.vehicleNetId then return end
    ...
    local vehEnt = NetworkGetEntityFromNetworkId(job.vehicleNetId)
    if not DoesEntityExist(vehEnt) then
        TriggerClientEvent('ox_lib:notify', src, { title = 'Giao Báo', description = 'Phải đi xe job mới giao được báo.', type = 'error' })
        return
    end
    if GetVehiclePedIsIn(GetPlayerPed(src), false) ~= vehEnt then
        TriggerClientEvent('ox_lib:notify', src, { title = 'Giao Báo', description = 'Phải đi xe job mới giao được báo.', type = 'error' })
        return
    end
...
RegisterNetEvent('delivery:server:returnJob', function()
    ...
    -- validate xe job phải đứng gần NPC
    if not job.vehicleNetId then
        TriggerClientEvent('ox_lib:notify', src, { title = 'Giao Báo', description = 'Không tìm thấy xe job.', type = 'error' })
        return
    end
    local vehEnt = NetworkGetEntityFromNetworkId(job.vehicleNetId)
    if not DoesEntityExist(vehEnt) then
        TriggerClientEvent('ox_lib:notify', src, { title = 'Giao Báo', description = 'Xe job không còn tồn tại.', type = 'error' })
        return
    end
    local vehCoords = GetEntityCoords(vehEnt)
    if #(vehCoords - npc) > Config.Job.returnRadius then
        TriggerClientEvent('ox_lib:notify', src, { title = 'Giao Báo', description = 'Đem xe về NPC mới nhận được tiền.', type = 'error' })
        return
    end
