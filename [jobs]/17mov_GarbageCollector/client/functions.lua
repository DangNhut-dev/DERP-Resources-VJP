...
function SetVehicle(_vehicle)
    ...
    -- wasabi_carlock: must be granted server-side. Trigger our own server
    -- event with netId + plate; the server validates proximity then calls
    -- exports.wasabi_carlock:GiveKey(src, plate).
    if GetResourceState("wasabi_carlock") == "started" then
        if vehicle and DoesEntityExist(vehicle) then
            TriggerServerEvent("17mov_GarbageCollector:sv:grantKey", NetworkGetNetworkIdFromEntity(vehicle), plate)
        end
    else
