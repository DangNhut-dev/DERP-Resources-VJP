...
lib.callback.register('derp-fishing:getCurrentZone', function()
...
---@param fish Fish
lib.callback.register('derp-fishing:itemUsed', function(bait, fish)
    ...
    CreateThread(function()
        ...
        local difficulty = #fish.skillcheck -- anchovy=2, shark=3

        exports['boii_minigames']:skill_bar({
            style = 'default',
            icon = 'fa-solid fa-fish',
            orientation = 2,
            area_size = math.max(10, 28 - (difficulty * 4)),     -- 2=20, 3=16, 4=12
            perfect_area_size = math.max(3, 8 - difficulty),      -- 2=6,  3=5,  4=4
            speed = 0.2 + (difficulty * 0.08),                    -- 2=0.36, 3=0.44, 4=0.52
            moving_icon = true,
            icon_speed = difficulty * 1.5,                        -- 2=3, 3=4.5, 4=6
        }, function(result)
            skillResult = result
        end)

        while skillResult == nil do
            Wait(0)
        end

        local success = skillResult == 'perfect' or skillResult == 'success'
