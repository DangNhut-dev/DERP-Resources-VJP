	...
	for i=1, #xPlayers, 1 do
		...
		if Player then
			table.insert(players, {
				...
				name		= Utils.Framework.getPlayerName(Player.PlayerData.citizenid)
			})
		end
	end
	return players
...
function Utils.Framework.giveAccountMoney(source,amount,account)
	local Player = QBCore.Functions.GetPlayer(source)
	Player.Functions.AddMoney(account, amount, GetCurrentResourceName())
end

function Utils.Framework.tryRemoveAccountMoney(source,amount,account)
	local Player = QBCore.Functions.GetPlayer(source)
	local money = Player.PlayerData.money[account]
...
function Utils.Framework.givePlayerItem(source,item,amount,metadata)
	...
	elseif Config.custom_scripts_compatibility.inventory == "qb-inventory" then
...
function Utils.Framework.givePlayerWeapon(source,item,amount,metadata)
	...
	if Config.custom_scripts_compatibility.mdt == "ps-mdt" then
		...
		local weapClass = 1
		local weapModel = ItemData.label
...
function Utils.Framework.hasWeaponLicense(source)
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.metadata["licences"] and Player.PlayerData.metadata["licences"]["weapon"] then
		return true
	else
		return false
	end
