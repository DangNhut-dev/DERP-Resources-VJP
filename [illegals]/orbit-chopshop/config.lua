Config = {}

Config.CallCops = false
Config.CoolDown = 1
Config.Email = false
Config.Fuel = 'cdn-fuel'

Config.StartLoc = vector4(815.09, -495.18, 29.62, 296.17)

Config.WheelItems = {
    [1] = {
        ["item"] = "carbon",
        ["amount"] = 3
    },
}

Config.DoorItems = {
    [1] = {
        ["item"] = "carbon",
        ["amount"] = 4
    },
}

Config.TrunkItems = {
    [1] = {
        ["item"] = "lockpick",
        ["amount"] = 1
    },
    [2] = {
        ["item"] = "at_clip_extended_pistol2",
        ["amount"] = 1
    },
}

Config.DeliveryCoords = {
    [1] = { coords = vector4(2055.7080, 3179.8103, 45.1689, 91.500389) },
    [2] = { coords = vector4(2351.7979, 3132.7310, 47.6015, 91.500389) },
    [3] = { coords = vector4(1565.4553, -2158.8508, 76.9222, 91.500389) },
    [4] = { coords = vector4(-458.0591, -1712.9053, 18.0516, 91.500389) },
}

Config.VehicleList = {
    [1]  = {vehicle = "Asterope"},
    [2]  = {vehicle = "Premier"},
    [3]  = {vehicle = "Primo2"},
    [4]  = {vehicle = "sultanrs"},
    [5]  = {vehicle = "Stanier"},
    [6]  = {vehicle = "Stratum"},
    [7]  = {vehicle = "Surge"},
    [8]  = {vehicle = "Tailgater"},
    [9]  = {vehicle = "Warrener"},
    [10] = {vehicle = "Washington"},
    [11] = {vehicle = "Asea"},
    [12] = {vehicle = "sultan2"},
    [13] = {vehicle = "Cog55"},
    [14] = {vehicle = "Cognoscenti"},
    [15] = {vehicle = "Emperor"},
    [16] = {vehicle = "Emperor2"},
    [17] = {vehicle = "Fugitive"},
    [18] = {vehicle = "Glendale"},
    [19] = {vehicle = "Ingot"},
    [20] = {vehicle = "Intruder"},
    [21] = {vehicle = "Buffalo"},
    [22] = {vehicle = "Kuruma"},
    [23] = {vehicle = "Schafter2"},
    [24] = {vehicle = "Schwarzer"},
    [25] = {vehicle = "Pigalle"},
    [26] = {vehicle = "Superd"},
    [27] = {vehicle = "Buffalo2"},
    [28] = {vehicle = "Felon"},
    [29] = {vehicle = "Jackal"},
    [30] = {vehicle = "Oracle"},
    [31] = {vehicle = "Sentinel"},
    [32] = {vehicle = "Dilettante"},
    [33] = {vehicle = "Blista"},
    [34] = {vehicle = "Blista2"},
    [35] = {vehicle = "Zion"},
    [36] = {vehicle = "Zion2"},
    [37] = {vehicle = "Feltzer2"},
    [38] = {vehicle = "Ninef"},
    [39] = {vehicle = "Fusilade"},
    [40] = {vehicle = "Jester"},
    [41] = {vehicle = "Carbonizzare"},
    [42] = {vehicle = "Sultan"},
    [43] = {vehicle = "Peyote"},
    [44] = {vehicle = "Buccaneer2"},
    [45] = {vehicle = "Picador"},
    [46] = {vehicle = "Virgo2"},
    [47] = {vehicle = "Brawler"},
}

Config.VehicleCoords = {
    [1] = {coords = vector4(-1074.953, -1160.545, 1.661577, 119.0), radarCoords = vector3(-1093.75, -1131.36, 2.16) },
    [2] = {coords = vector4(-1023.625, -890.4014, 5.202, 216.0399), radarCoords = vector3(-1022.15, -903.87, 2.13) },
    [3] = {coords = vector4(-1609.647, -382.792, 42.70383, 52.535), radarCoords = vector3(-1636.34, -374.05, 43.78) },
    [4] = {coords = vector4(97.57888, -1946.472, 20.27978, 215.933), radarCoords = vector3(102.8, -1982.58, 20.79) },
    [5] = {coords = vector4(437.5428, -1925.465, 24.004, 28.82286), radarCoords = vector3(436.09, -1875.0, 27.23) },
    [6] = {coords = vector4(438.4482, -1838.672, 27.47369, 42.8129), radarCoords = vector3(436.36, -1820.41, 27.98) },
}

Config.Locale = {
    ["Reminder"]            = "Hãy tháo các bộ phận của xe rồi vào lại trong xe để ép nát!",
    ["FarAway"]             = "Bạn đã đi quá xa khỏi chiếc xe. Xe đã được gửi đến bãi ép!",
    ["CoolDown"]            = "Hiện tại không có nhiệm vụ mới, vui lòng quay lại sau!",
    ["JobActive"]           = "Bạn vẫn chưa hoàn thành nhiệm vụ đang hoạt động!",
    ["Email"]               = "Tôi sẽ gửi thông tin vào email của bạn.",
    ["Ui"]                  = "Tôi sẽ tìm khách hàng cho bạn.",
    ["WrongVeh"]            = "Đây không phải là chiếc xe đúng yêu cầu!",
    ["FoundVeh"]            = "Tuyệt vời! Bạn đã tìm đúng chiếc xe!",
    ["ScrapBlip"]           = "Đang gửi vị trí bãi phế liệu đến GPS của bạn...",
    ["chop"]                = "~g~E~w~ - Bắt đầu tháo dỡ",
    ["remove"]              = "~g~E~w~ - Tháo ",
    ["destroy"]             = "~g~E~w~ - Phá hủy xe ",
    ["reqjob"]              = "~g~E~w~ - Yêu cầu thông tin xe.",
    ["Trunk"]               = "Cốp xe",
    ["Hood"]                = "Két nước",
    ["Frontleftdoor"]       = "cửa trước bên trái",
    ["Backleftdoor"]        = "cửa sau bên trái",
    ["Frontrightdoor"]      = "cửa trước bên phải",
    ["Backrightdoor"]       = "cửa sau bên phải",
    ["Frontleftwheel"]      = "bánh trước bên trái",
    ["Backleftwheel"]       = "bánh sau bên trái",
    ["Rightfrontwheel"]     = "bánh trước bên phải",
    ["Rightbackwheel"]      = "bánh sau bên phải",
    ["Wheel"]               = "Tháo bánh xe",
    ["Door"]                = "Tháo cửa xe",
    ["trunk"]               = "Tháo cốp xe",
    ["hood"]                = "Tháo két nước",
    ["crushing"]            = "Đang gửi xe đi ép nát",
    ["searching"]           = "Đang lục cốp xe",
    ["chopwheel"]           = "Đang cắt bánh xe...",
    ["chopdoor"]            = "Đang cắt cửa xe...",
    ["choptrunk"]           = "Đang cắt cốp xe...",
    ["chophood"]            = "Đang cắt nắp capo...",
    ["title"]               = "Trộm Xe",
    ["chop1"]               = "Tôi đang tìm xe cho bạn.",
    ["chop2"]               = "Khách hàng yêu cầu chiếc xe này.<br>Biển số: ",
    ["chop3"]               = "Di chuyển đến địa điểm tháo dỡ",
    ["chop4"]               = "Tháo toàn bộ bộ phận của xe",
    ["RadarSuccess"]        = "Bạn đã mở khóa xe thành công.",
    ["RadarError"]          = "Bạn không thể mở khóa chiếc xe này.",
}

Config.CarTable = {
    {name = Config.Locale["Trunk"],           coords = 0, vehBone = "boot",          distance = 1.5, chopped = false,  anim = "trunk",  destroy = 5, getin = 5},
    {name = Config.Locale["Hood"],            coords = 0, vehBone = "overheat",      distance = 0.8, chopped = false,  anim = "hood",   destroy = 4, getin = 4},
    {name = Config.Locale["Frontleftdoor"],   coords = 0, vehBone = "door_dside_f",  distance = 0.8, chopped = false,  anim = "door",   destroy = 0, getin = 0},
    {name = Config.Locale["Backleftdoor"],    coords = 0, vehBone = "door_dside_r",  distance = 0.8, chopped = false,  anim = "door",   destroy = 2, getin = 2},
    {name = Config.Locale["Frontrightdoor"],  coords = 0, vehBone = "door_pside_f",  distance = 0.8, chopped = false,  anim = "door",   destroy = 1, getin = -1},
    {name = Config.Locale["Backrightdoor"],   coords = 0, vehBone = "door_pside_r",  distance = 0.8, chopped = false,  anim = "door",   destroy = 3, getin = 1},
    {name = Config.Locale["Frontleftwheel"],  coords = 0, vehBone = "wheel_lf",      distance = 1.3, chopped = "cando", anim = "wheel1", destroy = 0, getin = 0},
    {name = Config.Locale["Backleftwheel"],   coords = 0, vehBone = "wheel_lr",      distance = 1.3, chopped = "cando", anim = "wheel2", destroy = 4, getin = 0},
    {name = Config.Locale["Rightfrontwheel"], coords = 0, vehBone = "wheel_rf",      distance = 1.3, chopped = "cando", anim = "wheel3", destroy = 1, getin = 0},
    {name = Config.Locale["Rightbackwheel"],  coords = 0, vehBone = "wheel_rr",      distance = 1.3, chopped = "cando", anim = "wheel4", destroy = 5, getin = 0},
}